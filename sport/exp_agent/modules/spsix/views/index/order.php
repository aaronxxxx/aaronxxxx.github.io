...
<?php use yii\widgets\LinkPager;?>
                <div class="pro_title ">极速六合彩注单查询(按注单)</div>
                <form name="form1" method="get" id="gridSearchForm" action="#/six/index/order" id="form1">
                            ...
                            <div class="trinput inputct pd10">
                                <div class="mgauto middle">
                                    ...
                                    <a  id="order_excel" onclick="orderexcel();">导出数据</a>
                            </div>
                            </div>
...
//导出注单到excel
    function orderexcel(){
        $('#order_excel').html('数据生成中...')
        $.ajax({
            type:'post',
            url:'/?r=six/index/order-excel',
            dataType:'json',
            data:$('#gridSearchForm').serialize(),
            success:function (data) {
                if(data.status==true){
                    layer.confirm('是否下载?',function () {
