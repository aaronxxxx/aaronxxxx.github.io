...
class IndexController extends BaseController{
    ...
    public function  actionOrderExcel(){
        ...
        $data = SixLotteryOrder::getOrderById($status,$user,$startTime,$endTime,$qishu,$orderSubNum);
...
//        $count = $dataProvider->getTotalItemCount();
        ...
        foreach ( $lists as $key=>$order )
        {
            if ( $n % $page_size === 0 )
            {
                ...
                $objectPHPExcel->getActiveSheet()->setCellValue('B1','六合彩注单');

                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('B2','六合彩注单');
                $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('B2','六合彩注单');
            ...
            $objectPHPExcel->getActiveSheet()->setCellValue('D'.($n+4) ,'六合彩');
        ...
        header('Content-Disposition:attachment;filename="'.'六合彩注单表-'.date("Y年m月j日").'.xls"');
...
//        $list = SixLotteryOrder::userOrderByOptimize($status,$user,$startTime,$endTime,$qishu,$pages->offset,$this->pageSize);
		...
		$result = SixLotteryOrder::userOrder($status,$user,$startTime,$endTime,$qishu);
        $count = $result->count();
        $pages = new Pagination(['totalCount' =>$count, 'pageSize' => $this->pageSize]);
...
//        }
		...
		$result = SixLotteryOrder::getOrderById($status,$user,$startTime,$endTime,$qishu,$orderSubNum);
