...
class OddsController extends BaseController{
    ...
    public function actionTest(){
        ...
        $objectPHPExcel->getActiveSheet()->setCellValue('B1',$name);
        $objectPHPExcel->getActiveSheet()->getColumnDimension('A')->setWidth(30);
        $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('B2','站点编号');
        $objectPHPExcel->getActiveSheet()->getColumnDimension('B')->setWidth(30);
        $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('C2','表名');
        $objectPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(30);
        $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('D2','总条数（数据）');
        $objectPHPExcel->getActiveSheet()->getColumnDimension('D')->setWidth(30);
        $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('E2','数据开始时间');
        $objectPHPExcel->getActiveSheet()->getColumnDimension('E')->setWidth(30);
        $objectPHPExcel->setActiveSheetIndex(0)->setCellValue('F2','数据最后时间');
        $objectPHPExcel->getActiveSheet()->setCellValue('B3' ,$name);
        $objectPHPExcel->getActiveSheet()->setCellValue('C3' ,$name);
        $objectPHPExcel->getActiveSheet()->setCellValue('D3' ,$name);
        $objectPHPExcel->getActiveSheet()->setCellValue('E3' ,$name);
        $objectPHPExcel->getActiveSheet()->setCellValue('F3' ,$name);
        ob_end_clean();
        ob_start();
        header('Content-Type : application/vnd.ms-excel');
        header('Content-Disposition:attachment;filename="'.'3088站点数据分析-'.date("Y年m月j日").'.xls"');
