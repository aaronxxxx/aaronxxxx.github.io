...
class SixLotteryOrder extends ActiveRecord {
     ...
     * @$qishu     六合彩期数
     */
    public static function userOrder($status,$user='',$startTime='',$endTime='',$qishu=''){
     ...
     * @$orderSubNum  子订单ID
     */
     public  static function getOrderById($status,$user='',$startTime='',$endTime='',$qishu='',$orderSubNum=''){
    ...
    public  static function getOrderByIdByOptimize($status,$user='',$startTime='',$endTime='',$qishu='',$orderSubNum='',$page=0,$pageSize=20){
        ...
        $sql1 =  (new Query())->select($select_str)->from('six_lottery_order');
        if($startTime){
        ...
        $sql2 = SixLotteryOrderSub::find()
            ->select('id')
            ->from('six_lottery_order_sub')
...
//            ->limit($pageSize);
        if($user){
            ...
            $list->from("six_lottery_order as o");
...
//------------------------------------------
    ...
    public  static function userOrderByOptimize($status,$user='',$startTime='',$endTime='',$qishu='',$page=0,$pageSize=20)
    {
        ...
        $result_sql  = SixLotteryOrder::find()
    ...
    /*
    * 订单作废
    * SixLotteryOrder 与 SixLotteryOrdersub manage_log user_list 都要修改
    * @$user 操作人名称
    * @$orderSubNum  子订单ID
    * @$reason 作废理由
    * @$status 订单状态 0:未结算 1:已结算 2:重新结算 3:已作废
    * SixLotteryOrdersub 修改子订单状态
    * manage_log  对作废理由，作废用户进行修改
    * user_list  对用户金额进行修改
    * SixLotteryOrder  当订单的所有子订单都作废的情况下修改主订单的订单状态为作废
    */
    public static function cancelOrder($orderSubNum='',$reason){
        $updateOrder  = $updateSub = $log = $updateMoney = 1;
        $orderSub = SixLotteryOrderSub::findOne(array('id'=>$orderSubNum));
        $betMoney = 0;
        if($orderSub){
...
//            $updateSub = $orderSub->save();//修改子订单状态
            $order = SixLotteryOrder::findOne(array('order_num'=>$orderNum));
            if($order){
                ...
                $orderSub->status = 3;
                $updateSub = $orderSub->save();//修改子订单状态
                $orderSubs = SixLotteryOrderSub::find(array('order_num'=>$orderNum))->select('count(id) as count')->andWhere('status !=:status',array(':status'=>3))->asArray()->one();
                if(!$orderSubs){
                    $order->status = 3;
    ...
    */
    public static function sixDetail($startTime,$endTime,$user_id,$group=''){
        $list = SixLotteryOrder::find()
            ...
            ->innerJoin("six_lottery_order_sub as o_sub","o.order_num=o_sub.order_num")
            ->innerJoin("user_list as u","o.user_id=u.user_id");
        $list->orWhere('o_sub.is_win is null');
        $list->orWhere('o_sub.is_win = 1');
        $list->orWhere('o_sub.is_win = 0');
        if(!empty($startTime)){
            $list->andWhere('o.bet_time >= :start_time',[':start_time'=>$startTime]);
        }
        if(!empty($endTime)){
    ...
    public  static function getOrderByOrderid($userId,$orderNum,$type){
        ...
        if($type!='六合彩' && $type!='彩票下注' && $type!='六合彩作废'){
