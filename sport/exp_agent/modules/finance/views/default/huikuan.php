<?php use yii\widgets\LinkPager;?>
 <form id="gridSearchForm" name="form1" method="GET" action="?r=finance/default/huikuan">
                ...
                if($data){
                    foreach ($data as $key=>$val){
                        ...
                        <tr align="center" onmouseover="this.style.backgroundColor = '#EBEBEB'" onmouseout="this.style.backgroundColor = '#ffffff'" style="background-color: rgb(255, 255, 255);">
                            ...
                            <td><a href="?r=finance/default/huikuan-detail&id=<?=$val['id'];?>&update=1" style="color:#FF9900;"><?=$val['order_num'];?></a><br><?=$val['update_time'];?></td>
                            <td><span style="color:#999999;"><?=$val['assets'];?></span><br><?=$val['order_value'];?><br><span style="color:#999999;"><?=$val['balance'];?></span></td>
                            <td><?=$val['zsjr'];?></td>
                            <td><a href="?r=finance/fund/look-money&username=<?=$val['user_name']?>&status=所有狀態&time_start=<?= urlencode($startime)?>&end_time=<?=  urlencode($endtime)?>" style="color:#FF9900;">查看財務</a></td>
                            ...
                            <td>
                                ...
                                <span><?=$statusArray[$val['status']];?></span>
                                    <br>
                                    <a href="?r=finance/default/huikuan-detail&id=<?=$val['id'];?>&update=1" style="color:#FF9900;">重新結算</a>
                                <?php }else if($val['status']=='審核中' || $val['status']=='未結算'){ $shenhe+=$val['order_value'];?>
                                    <span><a href="?r=finance/default/huikuan-detail&id=<?=$val['id'];?>&update=1" style="color:#FF9900;"><?=$statusArray[$val['status']];?></a></span>
                                <?php }else{ $success+=$val['order_value'];?>
                                    <span><a href="?r=finance/default/huikuan-detail&id=<?=$val['id'];?>&update=1" style="color:#006600;">匯款成功</a></span>
                                <?php }?>
                            </td>
                        </tr>
                <?php  }?>
