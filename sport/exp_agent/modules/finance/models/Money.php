...
//                ->all();
  ...
  public static function selectCk($mid){
    ...
    public static function updateCkFalse($mid){
        $r = 0;
        $money = Money::find()->where(['id'=>$mid])->andWhere(['status'=>'未結算'])->one();
        if($money){
            $money->status = '失敗';
 ...
 */
    ...
    public static function updateTixianStatus($status,$mid,$about='',$sxf=''){
        if($status ==1){
            $money = Money::find()->where(['id'=>$mid])->andWhere(['or',['status'=>'未結算'],['status'=>'審核中']])->one();
            $money->status = '成功';
            $money->sxf = $sxf;
            $money->about = $about;
        }elseif($status==0){
...
*/
    public static function selectAddMinusMoney(){
        $data = Money::find()
            ...
            ->from('money as m')
            ->innerJoin('user_list as u', 'm.user_id=u.user_id')
            ->where(['or',['m.type' => '後台充值'], ['m.type' => '後台扣款']]) //所有該會員的存款取款記錄
            ->andWhere(['>=','m.update_time',date('Y-m-d 00:00:00')])
            ->andWhere(['<=','m.update_time',date('Y-m-d 23:59:59')])
            ->andWhere('u.top_id = :agent_id',[':agent_id'=>Yii::$app->session['S_AGENT_ID']])
            ->asArray()
            ->all();
        return $data;
    }
