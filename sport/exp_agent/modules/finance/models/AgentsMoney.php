...
*/
    public static function selectAddMinusMoney(){
        $data = AgentsMoney::find()
            ...
            ->from('agents_money as m')
            ->innerJoin('agents_list as u', 'm.user_id=u.id')
            ->where(['or',['m.type' => '後台充值'], ['m.type' => '後台扣款'],['m.type' => '總代扣除代理金額']]) //所有該會員的存款取款記錄
            ->andWhere(['>=','m.update_time',date('Y-m-d 00:00:00')])
            ->andWhere(['<=','m.update_time',date('Y-m-d 23:59:59')])
            ->andWhere('u.agent_level = :agent_id',[':agent_id'=>Yii::$app->session['S_AGENT_ID']])
            ->asArray()
            ->all();
        return $data;
    }
