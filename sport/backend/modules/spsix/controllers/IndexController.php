...
//        $list = SpsixLotteryOrder::userOrderByOptimize($status,$user,$startTime,$endTime,$qishu,$pages->offset,$this->pageSize);
		...
		$result = SpsixLotteryOrder::userOrder($status,$user,$startTime,$endTime,$qishu,$excludeids);
        $count = $result->count();
        $pages = new Pagination(['totalCount' =>$count, 'pageSize' => $this->pageSize]);
...
//        }
		...
		$result = SpsixLotteryOrder::getOrderById($status,$user,$startTime,$endTime,$qishu,$orderSubNum,$excludeids);
	...
	public function actionUpdateResult(){
		...
		if($qishu && $ball1 && $ball2 && $ball3 && $ball4 && $ball5 && $ball6 && $ball7 && $dateTime){
			if($update){
				$saveModel = LotteryResultSplhc::findOne(array('qishu'=>trim($qishu)));
				if($saveModel){
				...
				$qishu_only = LotteryResultSplhc::findOne(array('qishu'=>trim($qishu)));
				if($qishu_only){
					return $this->out(false, "期号已经存在！");
				}else{
					$saveModel = new LotteryResultSplhc();
					$saveModel->qishu = trim($qishu);
					$saveModel->ball_1 = intval($ball1);
					$saveModel->ball_2 = intval($ball2);
					$saveModel->ball_3 = intval($ball3);
	...
	public function actionResult()
	{
		...
		if(isset($params['qishu']) && $params['qishu']){
			$saveData = LotteryResultSplhc::findOne(array('qishu'=>$params['qishu']));
			if(!$saveData){
				return "找不到您要编辑的期数！";
			}
			$update = 1;
		...
		$schedule = LotteryResultSplhc::find();
		if($qishu){
			$schedule->where(array('qishu'=>$qishu));
		}
		$pages = new Pagination(['totalCount' =>$schedule->count(), 'pageSize' => $this->pageSize]);
	...
	public function actionResultLog()
	{
		...
		$id = $this->getParam('id',0);
		if($id){
			$list = LotteryResultSplhc::findOne(array('id'=>trim($id)));
			if(empty($list)){
				return "找不到您要查看的期数!";
			}else{
				if($list->prev_text){
    ...
    public function actionResultDelete(){
	    ...
	    $id=$this->getParam('id',0);
	    if(!empty($id)){
            $result = LotteryResultSplhc::findOne(array('id'=>trim($id)))->delete();
            if($result){
                return $this->out(true,'删除成功');
            }else{
                return $this->out(false.'删除失败');
	 ...
	 */
	 public function actionDoState(){
		...
		$result = $SixCheckout->actionLotteryCheckout('SPSIX', 7, $qishu, $jsType);
		if(isset($result)){
			if($result['fail'] > 0){
 ...
 */
    ...
    public function actionTema() {
        $params = Yii::$app->request->get();
        $qishus = SpsixLotterySchedule::find()->select(array('qishu'))->groupBy(array('qishu'))->orderBy(array('qishu' => SORT_DESC))->asArray()->all();
