...
/**
 * SpsixLotteryOrderSub is the model behind the spsix_lottery_order_sub.
 */
class SpsixLotteryOrderSub extends ActiveRecord {
    ...
    public static function getOrderStatus($sub_id){
        return $result = SpsixLotteryOrderSub::find()->select('status')->where(['id'=>$sub_id])->asArray()->one();
    }
    /**
     *
     */
    public static function getTotalBetMoney($userid,$startTime='',$endTime=''){
        $result = SpsixLotteryOrderSub::find()
