...
<div class="pro_title ">
         极速六合彩注单查询(按用户)
            ...
            <form action="#/spsix/index/index" id="gridSearchForm" method="post" name="form1">       
                ...
                if($list){
                    foreach ($list as $key=>$val){?>
                        <tr align="center" style="background-color: rgb(255, 255, 255); line-height: 20px;" onmouseout="this.style.backgroundColor='#ffffff'" onmouseover="this.style.backgroundColor='#EBEBEB'">
                            <td valign="middle" height="40" align="center">极速六合彩</td>
                            <td valign="middle" align="center">
                                <a href="#/spsix/index/order&user_name=<?=$val['user_name']?>&start_time=<?=urlencode($startTime);?>&end_time=<?=urlencode($endTime);?>&qishu=<?=$qishu;?>&status=<?=$statu;?>" style="color: #F37605;" title="<?=$val['user_name']?>"><?=$val['user_name']?></a>(<?=$val['pay_name']?>)
                            </td>
