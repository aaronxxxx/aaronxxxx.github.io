...
class LiveReportService extends BaseService
{
    ...
    public function loadReportResult($startTime, $endTime, $userIds) {
        ...
        $sxzr_result = LiveOrder::getLiveOrderResult(0, $liveNames, $startTime, $endTime);  //真人
        $dzyy_result = LiveOrder::getLiveOrderResult(1, $liveNames, $startTime, $endTime);  //電子
        $pe_result = LiveOrder::getLiveOrderResultForPE(1, $liveNames, $startTime, $endTime);   //體育注單
        ...
        if ($pe_result["bet_count"] == 0) {
            $pe_result["bet_count"] = $pe_result["valid_bet_amount"] = $pe_result["bet_money"] = $pe_result["live_win"] = 0;
        }
    ...
    public function loadLiveAndGameResult($startTime, $endTime, $userIds)
    {
        ...
        $zr_result_vr = $this->_loadBetMoneyAndCount($startTime, $endTime, "VR", "转入", $userIdsStr);
        $zc_result_vr = $this->_loadBetMoneyAndCount($startTime, $endTime, "VR", "转出", $userIdsStr);
        $zr_result_pt = $this->_loadBetMoneyAndCount($startTime, $endTime, "PT", "转入", $userIdsStr);
        $zc_result_pt = $this->_loadBetMoneyAndCount($startTime, $endTime, "PT", "转出", $userIdsStr);
        $zr_result_ai = $this->_loadBetMoneyAndCount($startTime, $endTime, "AI", "转入", $userIdsStr);
        $zc_result_ai = $this->_loadBetMoneyAndCount($startTime, $endTime, "AI", "转出", $userIdsStr);
        ...
        $total_bet_count = $zr_result["bet_count"] + $zc_result["bet_count"] + $zr_result_agin["bet_count"] + $zc_result_agin["bet_count"]
            ...
            + $zr_result_mg["bet_count"] + $zc_result_mg["bet_count"] + $zr_result_ag_og["bet_count"] + $zc_result_ag_og["bet_count"]
            + $zr_result_og["bet_count"] + $zc_result_og["bet_count"] + $zr_result_kg["bet_count"] + $zc_result_kg["bet_count"]
            + $zr_result_vr["bet_count"] + $zc_result_vr["bet_count"] + $zr_result_ai["bet_count"] + $zc_result_ai["bet_count"];
        $total_bet_money = $zr_result["bet_money"] - $zc_result["bet_money"] + $zr_result_agin["bet_money"] - $zc_result_agin["bet_money"]
            ...
            + $zr_result_mg["bet_money"] - $zc_result_mg["bet_money"] + $zr_result_ag_og["bet_money"] - $zc_result_ag_og["bet_money"]
            + $zr_result_og["bet_money"] - $zc_result_og["bet_money"] + $zr_result_kg["bet_money"] - $zc_result_kg["bet_money"]
            + $zr_result_vr["bet_money"] - $zc_result_vr["bet_money"] + $zr_result_ai["bet_money"] - $zc_result_ai["bet_money"];
    ...
    private function _loadBetMoneyAndCount($dayStart, $dayEnd, $gType, $rType, $userIdsStr) {
        ...
        if ($gType == "AG") {
            ...
            if ($rType == "转入") {
                $sql_where.=" and zz_type='19' ";
            } elseif ($rType == "转出") {
                $sql_where.=" and zz_type='20' ";
            }
        } elseif ($gType == "PT") {
            if ($rType == "转入") {
                $sql_where.=" and zz_type='17' ";
            } elseif ($rType == "转出") {
                $sql_where.=" and zz_type='18' ";
            }
        } elseif ($gType == "AI") {
            if ($rType == "转入") {
                $sql_where .= " and zz_type='21' ";
            } elseif ($rType == "转出") {
                $sql_where .= " and zz_type='22' ";
            }
        }

        if ($gType == "All") {
            if ($rType == "转入") {
                $sql_where.=" and (zz_type='1' or zz_type='3' or zz_type='5' or zz_type='7' or zz_type='9' or zz_type='11' or zz_type='13' or zz_type='15' or zz_type='17' or zz_type='19' or zz_type='21')";
            } else{
                $sql_where.=" and (zz_type='2' or zz_type='4' or zz_type='6' or zz_type='8' or zz_type='10' or zz_type='12' or zz_type='14' or zz_type='16' or zz_type='18' or zz_type='20' or zz_type='22')";
            }
        }
