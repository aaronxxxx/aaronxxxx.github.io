...
//     	}
		...
		if($gametype!=""){
			$gametypearr=explode(',',$gametype);
			#YOPLAY SBTA 使用跟AG相同設定
			if($gametypearr[0] == 'YOPLAY' || $gametypearr[0] == 'SBTA' || $gametypearr[0] == 'Bb_Sport' || $gametypearr[0] == 'AG_HUNTER' ){
				$gametypearr[0] = 'AG';
			}
    ...
    {
        $query = LiveUser::find()->select(['live_username'])->distinct()->where(['in', 'user_id', $userIds]);
