...
class LiveOrder extends ActiveRecord {
    ...
    public static function getLiveOrderCount($userstr=null,$gametype=null,$start_order_time=null,$end_order_time=null){
		...
		->from('live_order')
		->where(['between', 'order_time', $start_order_time, $end_order_time])
		->andWhere(['not in', 'live_type', $eGames]);
	...
	public static function getPEOrderCount($userstr=null,$gametype=null,$start_order_time=null,$end_order_time=null){

        $eGames = array_values(Yii::$app->getModule('live')->params['pe']);
	
		$query=(new Query())->select('COUNT(*) as count,sum(bet_money) as bet_money,sum(valid_bet_amount) as valid_bet_amount,sum(live_win) as live_win')
		->from('live_order')->where(['between', 'order_time', $start_order_time, $end_order_time]);
	...
	public static function getEgameOrderCount($userstr=null,$gametype=null,$start_order_time=null,$end_order_time=null){
		...
		$query=(new Query())->select('COUNT(*) as count,sum(bet_money) as bet_money,sum(valid_bet_amount) as valid_bet_amount,sum(live_win) as live_win')
		->from('live_order')->where(['between', 'order_time', $start_order_time, $end_order_time]);
		if($gametype != 'SBTA' || $gametype != 'ALL'){// 不等于ALL SBTA平台全部印出
			$query->andWhere(['in', 'live_type', $eGames]);
		}elseif($gametype == 'ALL')
		{
			$query->andWhere(
				['or',
		...
		if($userstr!=null||$userstr!=""){
			$liveuserarr=[];
			$userarr=explode(',',$userstr);
			$userlists = UserList::find()->where(['in','user_name',$userarr])->with('liveUser')->asArray()->all();
			
			foreach ($userlists as $key=>$userlist){
				$liveusers=$userlists[$key]['liveUser'];
				
				$liveusers = array_unique(array_column($liveusers,"live_username"));
				foreach ($liveusers as $key => $value) {
					array_push($liveuserarr,$value);
				}
			}
			$query->andWhere(['in','live_username',$liveuserarr]);
		}
	...
	public static function getEgameOrderSum($userstr=null,$gametype=null,$start_order_time=null,$end_order_time=null){
		...
		$query=(new Query())->select("sum(bet_money) as bm ,sum(valid_bet_amount) as vm ,sum(live_win) as lm")
		->from('live_order')->where(['between', 'order_time', $start_order_time, $end_order_time]);
		// if($gametype != 'SBTA' || $gametype != 'ALL'){//SBTA平台全部印出
		$query->andWhere(['in', 'live_type', $eGames]);
		// }elseif($gametype == 'ALL')
		// {
		// 	$query->andWhere(
	...
	public static function getPEOrderSum($userstr=null,$gametype=null,$start_order_time=null,$end_order_time=null){
	...
	public static function getPEOrder($userstr=null,$gametype=null,$start_order_time=null,$end_order_time=null,$pageoffset,$pagelimit){
        $eGames = array_values(Yii::$app->getModule('live')->params['pe']);
		$query=static ::find()->andwhere(['between', 'order_time', $start_order_time, $end_order_time]);
		if($gametype != 'SBTA' && $gametype != ''){	//SBTA平台全部印出
			$query->andWhere(['in', 'live_type', $eGames]);
		}
		if($gametype == '')
		{
    ...
    public static function getLiveOrderResult($code, $live_name_arr = [], $s_time, $e_time){
		...
		//if(count($live_name_arr) > 0){  
		$r->andWhere(['in','live_username',$live_name_arr]);//=0 為找不到任何資料
		//}
        if($code === 0){
            $r->andWhere(
				['and',
				['<>','game_type', 'SBTA'],
				['<>','game_type', 'Bb_Sport'],
				[ 'not in','live_type', Yii::$app->getModule('live')->params['egametype'] ]
				]
			);
	...
	//抓取體育注單
	public static function getLiveOrderResultForPE($code, $live_name_arr = [], $s_time, $e_time){
        $r = LiveOrder::find()
            ->select(['count(id) as bet_count','sum(bet_money) as bet_money','sum(valid_bet_amount) as valid_bet_amount','sum(live_win) as live_win'])
            ->from('live_order')
            ->where(['and', ['>=', 'order_time', $s_time], ['<=', 'order_time', $e_time]]);
