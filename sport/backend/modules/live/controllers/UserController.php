...
class UserController extends BaseController
{
	...
	{
        $start_order_time = '1900-01-01 00:00:00';// $this->getParam('s_time', date('Y-m-d 00:00:00'));
        $end_order_time = date ('Y-m-d H:i:s');//$this->getParam('e_time', date('Y-m-d H:i:s'));
		$gametype = $this->getParam('game_type', "AG,AGIN,DS,AG_BBIN,AG_MG,OG,KG,VR,YOPLAY,PT,AI");//2019 任務000095 移除AG_OG
		$userstr = trim($this->getParam('user_str', ''));

	    if ($gametype == "ALL") {
			$gametype = "AG,AGIN,DS,AG_BBIN,AG_MG,OG,KG,VR,YOPLAY,PT,AI";
		}
	...
	public function actionTransfer($uid,$hall,$type)
	{
		$rsuser=UserList::getUserById($uid);
		$data=['rsuser'=>$rsuser,'hall'=>$hall,'type'=>$type];
