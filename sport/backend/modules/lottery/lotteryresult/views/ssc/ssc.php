...
<form name="form1" method="post" id="form1" action="?r=lotteryresult/ssc/operation&id=<?=$id?>&action=<?=$id>0 ? 'edit' : 'add'?>&type=<?=$lottery_type?>&s_time=<?=$query_time?>&qishu_query=<?=$qishu_query?>">
<div class="tabft13 trinput  zudan spanmg0">
<p>
    <span>彩票类别：</span>
    <span><strong><?=$_GET['type']?></strong></span>
    <button type="button" onclick="javascript:location.href='#/lotteryresult/batch&type=<?= $lottery_type ?>'">批量开奖</button>
</p>
<p>
    <span>开奖期号：</span>
    ...
    <span>开奖号码：</span>
     <span>
        ...
        <select name="ball_4" id="ball_4">
            <option value="0" <?php if(isset($lists) && count($lists) > 0){if($lists['0']['ball_4'] == 0){echo 'selected';};}?>>0</option>
        ...
        <select name="ball_5" id="ball_5">
            <option value="0" <?php if(isset($lists) && count($lists) > 0){if($lists['0']['ball_5'] == 0){echo 'selected';};}?>>0</option>
...
<p>
    ...
    <span align="left" bgcolor="#FFFFFF"><input type="button" class="form_ajax_submit_btn mgl61" data-targetid="form1" data-redirect="#/lotteryresult/ssc/list&status=0&type=<?=$_GET['type']?>&<?=time()?>"  name="submit" value="确认发布"></span>
</p>
</div>
</form>

<form name="form2" method="get" id="gridSearchForm" action="#/lotteryresult/ssc/list">
    <input name="type" type="hidden" value="<?= $_GET['type']?>" />
    <input name="p" type="hidden" value="1" />
    <div class="tabft13 trinput resulttable zudan pd10">
        <div align="left">
            开奖期号：
            <input name="qishu_query" type="text" id="qishu_query" value="<?php if(isset($_GET['qishu_query'])){echo $_GET['qishu_query'];} ?>" size="20" maxlength="12"/>
            &nbsp;&nbsp;日期：
    ...
    <tr class="namecolor">
        ...
        if(isset($rows) && count($rows) > 0){
            foreach($rows as $key =>$val){
                ...
                $hm[]		= BuLing($val['ball_4']);
                $hm[]		= BuLing($val['ball_5']);
        ?>
        <tr align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#fff'" style="background-color:rgb(255, 255, 255); line-height:20px;">
            ...
            <td align="center" valign="middle"><img src="/public/lottery/images/ball_5/<?=$val['ball_4']?>.png"></td>
            <td align="center" valign="middle"><img src="/public/lottery/images/ball_5/<?=$val['ball_5']?>.png"></td>
            ...
            <td><?php
                ...
                ?></td>
            <td>
                <a href="#/lotteryresult/ssc/edit&id=<?=$val['id']?>&action=edit&type=<?=$lottery_type?>&qishu=<?= $val['qishu']?>" >编辑</a>
            ...
            $('.js_type').click(function(){
                ...
                $.ajax({
                 ...
                 success: function(data){
                     if(data=='0'){
                         alert('结算成功');
                         window.location.href='#/lotteryresult/ssc/list&status=0&type='+$lottery+'&n_time=<?=time();?>';
                     }else if(data == '-1'){
                         ...
                         alert('注单不存在');
                         window.location.href='#/lotteryresult/index/ball5&status=0&type='+$lottery+'&n_time=<?=time();?>';
                     }
                 }
               });
               return false;
