...
?>
    <script language="javascript" src="/public/lottery/js/result_ssrc.js"></script>
        <form name="form1" id="form1" method="post" action="?r=lotteryresult/ssrc/operation&id=<?=$id?>&action=<?=$id>0 ? 'edit' : 'add'?>&s_time=<?=$query_time?>&qishu_query=<?=$qishu_query?>">
            <div class=" trinput resulttable w836 tabft13 spanmg0">
            <p>
                <span>彩票类别：</span>
                <span><strong>极速赛车</strong></span>
                <button type="button" onclick="javascript:location.href='#/lotteryresult/batch&type=极速赛车'">批量开奖</button>
            </p>
            <p>
                <span>开奖期号：</span>
            ...
            <p>
               <span><input type="button" class="form_ajax_submit_btn mgl61" data-targetid="form1" data-redirect="#/lotteryresult/ssrc/list&status=0&type=type=极速赛车&<?=time()?>" name="submit" value="确认发布"></span>
            </p>
        </div>
        </form>
        <form name="form2" id="gridSearchForm" method="get" action="#/lotteryresult/ssrc/list">
            ...
            if(isset($rows) && count($rows) > 0){
                foreach($rows as $key =>$val){
                    ...
                    <tr align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#fff'" style="background-color:rgb(255, 255, 255); line-height:20px;">
                        ...
                        <td>
                            <a href="#/lotteryresult/ssrc/edit&id=<?=$val['id']?>&qishu=<?= $val['qishu']?>" >编辑</a>
            ...
            $('.js_type').click(function(){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        if(data=='0'){
                            alert('结算成功');
                            window.location.href='#/lotteryresult/ssrc/list&status=0&type=极速赛车&n_time=<?=time();?>';
                        }else if(data == '-1'){
                            ...
                            alert('注单不存在');
                            window.location.href='#/lotteryresult/index/ssrc&status=0&type=极速赛车&n_time=<?=time();?>';
                        }
                    }
                });
                return false;
