...
?>
<script language="javascript" src="/public/lottery/js/result_gd11.js"></script>
<form name="form1" method="post" id="form1" action="?r=lotteryresult/gd11/operation&id=<?=$id?>&action=<?=$id>0 ? 'edit' : 'add'?>&s_time=<?=$query_time?>&qishu_query=<?=$qishu_query?>">

    <div class=" trinput resulttable zudan tabft13 spanmg0">
<p>
    <span>彩票类别：</span>
    <span><strong>广东11选5</strong></span>
</p>
<p>
    <span>开奖期号：</span>
...
<p>
   ...
   <span><input type="button" class="form_ajax_submit_btn mgl61" data-targetid="form1" data-redirect="#/lotteryresult/gd11/list&status=0&type=type=广东11选5&<?=time()?>" name="submit" value="确认发布"></span>
</p>
</div>
</form>

<form name="form2" onSubmit="return queryLottery();" id="gridSearchForm" method="get" action="#/lotteryresult/gd11/list">
...
<table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12 skintable line35">
    <tr class="namecolor">
        ...
        <tr align="center" onMouseOver="this.style.backgroundColor='#EBEBEB'" onMouseOut="this.style.backgroundColor='#fff'" style="background-color:rgb(255, 255, 255); line-height:20px;">
            <td height="25" align="center" valign="middle">广东十一选五</td>
            ...
            <td><?php
                ...
                ?></td>
            <td>
                <a href="#/lotteryresult/gd11/edit&id=<?=$val['id']?>&qishu=<?= $val['qishu']?>">编辑</a>
            ...
            $('.js_type').click(function(){
                ...
                $.ajax({
                    ...
                    success: function(data){
                        if(data=='0'){
                            alert('结算成功');
                            window.location.href='#/lotteryresult/gd11/list&status=0&type=广东11选5&n_time=<?=time();?>';
                        }else if(data == '-1'){
                            ...
                            alert('注单不存在');
                            window.location.href='#/lotteryresult/index/gd11x5&status=0&type=广东11选5&n_time=<?=time();?>';
                        }
                    }
                });
                return false;
