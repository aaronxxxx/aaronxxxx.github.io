...
use app\modules\lottery\lotteryresult\model\LotteryResultGxsf;
use app\common\data\Pagination;
use app\common\base\BaseController;
/*
 * 注单控制器
 */
class GxsfcController extends BaseController{
    public $layout=false;
	private $pagesize = 20;
    ...
    public function actionList(){
        ...
        $cqsfc = LotteryResultGxsf::GxsfcList($time,$qishu);
        ...
        return $this->render('gxsfc',['id'=>$id,'qishu_query'=>$qishu,'query_time'=>$time,'rows'=>$lists,'pages'=>$pages]);
    }
    //新增修改操作
    public function actionOperation(){
        ...
        if($id==0){
            ...
            $cqsfc_qihao = LotteryResultGxsf::Qishi($where_cqsfc);
            if($cqsfc_qihao && $cqsfc_qihao['0']["id"] > 0){
                return $this->out('false','该期彩票结果已存在，请查询后编辑。');
            }else{
                $saveModel = new LotteryResultGxsf();
                ...
                $saveModel->save();
                $str = 'gxsfc'.'新增期數:'.$qishu.'號碼:'.$_POST["ball_1"].','.$_POST["ball_2"].','.$_POST["ball_3"].','.$_POST["ball_4"].','.$_POST["ball_5"];
                ManageLog::saveLog(Yii::$app->getSession()->get('S_USER_NAME'),$str);
                return $this->out(true, "添加成功!");
            }
        }elseif($id>0){
            $where['id'] = $id;
            $lists = LotteryResultGxsf::Up($where);
            $saveModel = new LotteryResultGxsf();
    ...
    public function actionEdit(){
        ...
        $resule_id = LotteryResultGxsf::Edit($id);
        $pages = new Pagination(['totalCount' =>'0', 'pageSize' => '20']);
        $lists = $resule_id->limit('1')->asArray()->all();
        return $this->render('gxsfc',['id'=>$id,'qishu_query'=>$qishu_query,'query_time'=>$query_time,'lists'=>$lists,'pages'=>$pages]);
    }
}
