...
use app\modules\lottery\lotteryresult\model\LotteryResultMlaft;
use app\common\data\Pagination;
use app\common\base\BaseController;
/*
 * 注单控制器
 */
class MlaftController extends BaseController{
	private $pagesize = 20;
    ...
    public function actionList(){
        ...
        $cqsfc = LotteryResultMlaft::Pk10List($time,$qishu);
        ...
        return $this->render('mlaft',['id'=>$id,'qishu_query'=>$qishu,'query_time'=>$time,'rows'=>$lists,'pages'=>$pages]);
    }
    //新增修改操作
    public function actionOperation(){
        ...
        if($id==0){
            ...
            $cqsfc_qihao = LotteryResultMlaft::Qishi($where_cqsfc);
            if($cqsfc_qihao && $cqsfc_qihao['0']["id"] > 0){
                return $this->out('false','该期彩票结果已存在，请查询后编辑。');
            }else{
                $saveModel = new LotteryResultMlaft();
                ...
                $saveModel->save();
                $str = 'mlaft'.'新增期數:'.$qishu.'號碼:'.$_POST["ball_1"].','.$_POST["ball_2"].','.$_POST["ball_3"].','.$_POST["ball_4"].','.$_POST["ball_5"].','.$_POST["ball_6"].','.$_POST["ball_7"].','.$_POST["ball_8"].','.$_POST["ball_9"].','.$_POST["ball_10"];
                ManageLog::saveLog(Yii::$app->getSession()->get('S_USER_NAME'),$str);
                return $this->out(true, "添加成功!");
            }
        }elseif($id>0){
            $where['id'] = $id;
            $lists = LotteryResultMlaft::Up($where);
            $saveModel = new LotteryResultMlaft();
    ...
    public function actionEdit(){
        ...
        $resule_id = LotteryResultMlaft::Edit($id);
        $pages = new Pagination(['totalCount' =>'0', 'pageSize' => '20']);
        $lists = $resule_id->limit('1')->asArray()->all();
        return $this->render('mlaft',['id'=>$id,'qishu_query'=>$qishu_query,'query_time'=>$query_time,'lists'=>$lists,'pages'=>$pages]);
    }
}
