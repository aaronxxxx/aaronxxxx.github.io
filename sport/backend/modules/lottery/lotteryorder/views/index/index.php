...
<div class="pro_title pd10">彩票注单查询</div>
    ...
    <form name="form2" method="post" action="/?r=lotteryorder/index/plzuofei">
        <table width="100%" border="0" cellpadding="5" id="orderlist" cellspacing="1" class="font12 skintable line35">
            ...
            ?>
                <?php if (($rows['status'] == 1 || $rows['status'] == 2) && $rows['is_win'] > 0) {?>
                <tr align="center" style="background-color:#f75050" line-height:20px>
                <?php }else {?>
                ...
                <?php }?>
                    ...
                    <td>
                        ...
                        ?>
                        <?php if (($rows['status'] == 1 || $rows['status'] == 2) && $rows['is_win'] > 0) {?>
                        <a style="color: #086913;" href="#/lotteryorder/index/lotteryuser&type=ALL_LOTTERY&js=0,1,2,3&username=<?=$username?>&s_time=<?= urlencode(isset($_GET["s_time"])?$_GET["s_time"]:date('Y-m-d 00:00:00')) ?>&e_time=<?= urlencode(isset($_GET["e_time"])?$_GET["e_time"]:date('Y-m-d H:i:s')) ?>&qishu=<?= isset($_GET["qishu"])?$_GET["qishu"]:''?>"><?= $username ?></a>
                        <?php }else {?>
                    ...
                    <td class="win_status">
                        ...
                        <?php } ?>
                        <?php if ($rows['status'] == 1) { ?><font color="#086913">已結算</font><?php } ?>
