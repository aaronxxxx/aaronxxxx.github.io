 ...
 * 六合彩报表明细
 */
class SixReportService extends BaseService {
    /**
     * 六合彩报表明细 汇总
    ...
    public function sixDetail( $userInArray=array(),$startTime='',$endTime='',$page=0,$pageSize=20){
        $list = SixLotteryOrder::sixDetail($startTime, $endTime, $userInArray);
        $count =  $list->count();
    ...
    public function sixDetailUser( $userInArray=array(),$startTime='',$endTime='',$page=0,$pageSize=20){
        $list = SixLotteryOrder::sixDetail($startTime, $endTime, $userInArray,'user');
        $count =  $list->count();
        //$count = count($list->asArray()->all());
        $data = $list->offset($page)->limit($pageSize)->asArray()->all();
        $result['data'] = $data;
    ...
    public function sixDetailNum( $userInArray=array(),$startTime='',$endTime='',$page=0,$pageSize=20){
        $list = SixLotteryOrder::sixDetail($startTime, $endTime, $userInArray,'num');
        $count =  $list->count();
        //$count = count($list->asArray()->all());
        $data = $list->offset($page)->limit($pageSize)->asArray()->all();
        $result['data'] = $data;
    ...
    public function sixDrawSum($userid,$startTime='',$endTime=''){
           $data = SixLotteryOrder::getDrawSum($userid,$startTime,$endTime);
    ...
    public function sixTotalBetMoney($userid,$startTime='',$endTime=''){
            $data = SixLotteryOrderSub::getTotalBetMoney($userid,$startTime,$endTime);
            return $data;
    }
}
