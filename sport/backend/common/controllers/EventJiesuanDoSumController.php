...
class EventJiesuanDoSumController extends Controller
{
    ...
    public function actionAgentsJiesuan($agent, $s_time, $e_time)
    {
        ...
        foreach ($arr as $key => $val) {
            if (($val['s_time'] <= substr($s_time, 0, 10)) && (substr($s_time, 0, 10) <= $val['e_time'])) {
                return "開始時間已經有結算過，請查詢後再結算。";
            }

            if (($val['s_time'] <= substr($e_time, 0, 10)) && (substr($e_time, 0, 10) <= $val['e_time'])) {
                return "結束時間已經有結算過，請查詢後再結算。";
            }
        }

        $res = AgentsList::getAgentsNewsBySumID($agent['id']);

        foreach ($res as $key => $val) {
            // 賽事結算
            $rows_event = AgentsList::jsAgentEventNoStatus($val['id'], $s_time, $e_time);
            ...
            if ($rows_event) {
                ...
                $money += ($agent['refunded_scale'] - $val['refunded_scale']) / 100 * $rows_event['bet_money_total'];
            }
        }
