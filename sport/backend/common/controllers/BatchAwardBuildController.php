...
class BatchAwardBuildController extends Controller
{
    ...
    #取得開獎球號
    #snumber 指定開獎方式
    public static function getBall($startNumber, $endNumber, $snumber = '')
    {
        if ($snumber != '') {
            $pool = [];
     ...
     * @return: qishu:期數
     */
    public static function getQishu($dateTime, $lottery_frequency, $type, $name)
    {
        switch ($type) {
            case 'TJ':
    ...
    #取得開獎時間
    public static function getDateTime($qishu, $type)
    {
        switch ($type) {
            case 'TJ':
    ...
    public static function awardBuild($type, $config, $batch)
    {
        ...
        if ($data == '') {
            // 假設從未開過獎 抓5分鐘前的期數
            $strQishu = self::getQishu(date('Y-m-d H:i:00', strtotime($awardTime . ' -5 minute')), $config['lottery_frequency'], $type, $config['name']);
        } else {
            $strQishu = $data['qishu'];
        ...
        $endQishu = self::getQishu($awardTime, $config['lottery_frequency'], $type, $config['name']);
        ...
        $strDate = date('Y-m-d 00:00:00', strtotime(self::getDateTime($strQishu, $type)));
        $endDate = date('Y-m-d H:i:s', strtotime(self::getDateTime($endQishu, $type)));
            ...
            case 'TJ': // 計算勝率後開獎
                for ($d = 1; $d <= $dateDiff; $d++) {
                    ...
                    for ($i = $strNum; $i <= $endNum; $i++) {
                        ...
                        $dateTime = self::getDateTime($qishu, $type);
                ...
                for ($d = 1; $d <= $dateDiff; $d++) {
                    ...
                    for ($i = $strNum; $i <= $endNum; $i++) {
                        ...
                        if ($endQishu >= $qishu) {
                            ...
                            //self::saveByRand($qishu, $dateTime, $config);    //亂數開獎
                            //因應預開彩，先行檢測是否開獎
                            $temp = "SELECT * FROM " . $config['table'] . " where qishu ='" . $qishu . "'";
                            $check = $db->createCommand($temp)->queryOne();
    ...
    private static function save($qishu, $awardTime, $type, $config)
    {
        ...
        switch ($build_type) {
            case 0:
                $result = self::simulation01($config, $type);
                break;
            case 1:
    ...
    private static function simulation01($config, $type)
    {
        $rand_array = [];
        $result = [];
        $db = Yii::$app->db;
    ...
    private static function simulation02($qishu, $awardTime, $type, $config)
    {
        ...
        if (count($data) > 0) {
            ...
            for ($i = 1; $i <= 10; $i++) {
                ...
                $assign_ball = self::str2num($quick_type);
                $assign_num = self::getBall($config['startNumber'], $config['endNumber'], $assign_array['snumber']);
    ...
    #中文轉數字
    private static function str2num($quick_type = '')
    {
        switch ($quick_type) {
            case '冠军':
                $num = 1;
