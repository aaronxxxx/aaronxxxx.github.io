...
<?php
	...
	$orderNo = $_POST['user_name'].date('YmdHis').mt_rand(100,999);			//訂單號嗎	
	// 往数据库添加数据
	//$config->order_bak($orderNo, $_POST['totalAmount'], $uid, $_POST['user_name'], $bankCode);
	$url = $configext["config"]["request_gateway"];
	$token = trim($configext["config"]["token"]);
	//$key = trim($configext["config"]["key"]);
	...
	switch($payType){
		...
		case 'WAP':
			...
			$Service = new Service();
			$order = $Service->buildForm($payType, $parameter, $url, "GET", $configext['config'], $token);
