...
class Luzhu{
	...
	//ball:中奖号码的个数
	//ball_count:球的总和
	public function __construct($list,$which=1,$cmax=10,$ball=10,$type=0,$ball_count){
		if(is_array($list)){
			krsort($list);
			$this->lists=array_values($list);
		}
		$this->count_middle = ((float)$ball_count)/2;
	...
	public function cout(){//露珠图数据处理
		...
		$s_arr=array();
		$list=$this->lists;
		$isSingle=$list[0]['ball_count']%2;
		$isBig=($list[0]['ball_count'] - $this->count_middle>0)?1:0;
		$str=array();
		$max=count($list)-1;
		foreach($list as $k=>$v){
			$cilent_s=$v['ball_count']%2;
			$str[]=$cilent_s;
			$m=$v['ball_count']-$this->count_middle;
			$cilent_b=($m>0)?1:0;
...
//---------以下是辅助函数----------------------------------------------
	private function mid($m_arr){//翻转弯曲函数——核心函数
		...
		$s_arr=array();
		$n_arr=array();
		$size=$this->size($m_arr);  //返回横向宽度 和 最大高度

		for($i=0;$i<$size['w'];$i++){//翻转、填满
			for($j=0;$j<$size['h'];$j++){
		...
		for($i=5;$i>4;$i--){
			for($j=0;$j<$size['w'];$j++){
				if(isset($n_arr[$i][$j])&&$n_arr[$i][$j]){
					if($this->type==0){//单双
						$cilent=$n_arr[$i][$j]['ball_count']%2;
					}else{//大小
						$cilent=($n_arr[$i][$j]['ball_count']-$this->count_middle>0)?1:0;
					}
					if($sign==-1){
						$sign=$cilent;
					}else{
	...
	private function modify($arr){//固定输出6x35矩阵函数
		...
		for($i=0;$i<6;$i++){
			for($j=0;$j<$size['w'];$j++){
				$v=$n_arr[$i][$j];
				if($v){
					$k_arr[$i][$j]['issingle']=$v['ball_count']%2;
					$m=$v['ball_count']-$this->count_middle;
	...
	public function cout_bjkl8(){//露珠图数据处理
		if(!is_array($this->lists)||(count($this->lists)==0)){
			$arr=array();
			for($i=0;$i<6;$i++){
				for($j=0;$j<35;$j++){
		...
		$n_arr=$this->mid($m_arr);
		//	print_r($n_arr);exit;
		return $this->modify_bjkl8($n_arr);
	}
	private function modify_bjkl8($arr){//固定输出6x35矩阵函数
		$arr=array_slice($arr,0,6);//截取掉多余尾部
		$size=$this->size($arr);
		$length=($size['w']<35)?35:$size['w'];
		$tmp_arr=array();
