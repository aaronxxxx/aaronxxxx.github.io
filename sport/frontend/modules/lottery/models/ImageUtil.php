...
namespace frontend\modules\lottery\models;
...
class ImageUtil
{
    public function CreateImage($gType,$gTypeZhName,$qiuhao,$wanfa,$lottery_number,$bet_money_one,$fs_money,$bet_rate,$datereg_sub)
    {
		$url = Yii::getAlias('@resource');
		$tm = date('Y-m-d H:i:s', time());
		$height = 26;
		error_reporting(E_ALL & ~E_NOTICE);  // 生成图片时候出错notice
		$betInfoIm = $this->getName($wanfa, $gType);
		$width = $this->str_leng($gTypeZhName . '=' . $lottery_number . '=' . $qiuhao . '=' . $betInfoIm . '=' . $bet_money_one . '=' . $fs_money . '=' . $bet_rate . '=' . $tm);
		...
		$tm_c = imagecolorallocate($im, 0, 0, 0);
		$fnt = 'order/ttf/simhei.ttf';
		imagettftext($im, 10, 0, 7, 18, $sort_c, $fnt, $gTypeZhName);
		imagettftext($im, 10, 0, $this->str_leng($gTypeZhName . '=='), 18, $name_c, $fnt, $lottery_number);
		imagettftext($im, 10, 0, $this->str_leng($gTypeZhName . $lottery_number . '==='), 18, $guest_c, $fnt, $qiuhao);
		imagettftext($im, 10, 0, $this->str_leng($gTypeZhName . $lottery_number . $qiuhao . '===='), 18, $info_c, $fnt, $betInfoIm);
		imagettftext($im, 10, 0, $this->str_leng($gTypeZhName . $lottery_number . $qiuhao . $betInfoIm . '====='), 18, $info_c, $fnt, $bet_money_one);
		imagettftext($im, 10, 0, $this->str_leng($gTypeZhName . $lottery_number . $qiuhao . $betInfoIm . $bet_money_one . '======'), 18, $money_c, $fnt, $fs_money);
		imagettftext($im, 10, 0, $this->str_leng($gTypeZhName . $lottery_number . $qiuhao . $betInfoIm . $bet_money_one . $fs_money . '======='), 18, $money_c, $fnt, $bet_rate);
		imagettftext($im, 10, 0, $this->str_leng($gTypeZhName . $lottery_number . $qiuhao . $betInfoIm . $bet_money_one . $fs_money . $bet_rate . '========'), 18, $tm_c, $fnt, $tm);
		imagerectangle($im, 0, 0, $width - 1, $height - 1, $font);

		if (!is_dir($url . "/order/")) {
			mkdir($url . "/order/");
		}
		if (!is_dir($url . "/order/" . substr($datereg_sub, 0, 8))){
			mkdir($url . "/order/" . substr($datereg_sub, 0, 8));
		}
		imagejpeg($im,$url .  '/order/' . substr($datereg_sub, 0, 8) . '/' . $datereg_sub . '.jpg');
		imagedestroy($im);
    }
    ...
    function getName($contentName, $gType ,$rTypeName="",$quickType=""){
    	...
    	if($gType=="GD11"){
    		$name = $this->getNameGd11($contentName);
    	}elseif($gType=="BJPK"){
    		$name = $this->getNameBJPK($contentName);
    	}elseif($gType=="BJKN"){
    		$name = $this->getNameBJKN($contentName);
    	}elseif($gType=="GXSF"){
    		$name = $this->getNameGXSF($contentName);
    	}elseif($gType=="GDSF"){
    		$name = $this->getNameGDSF($contentName);
    	}elseif($gType=="TJSF"){
    		$name = $this->getNameTJSF($contentName);
    	}elseif($gType=="CQ" || $gType=="TJ" || $gType=="JX"){
    		$name = $this->getNameb5($contentName);
    	}elseif($gType=="D3" || $gType=="P3" || $gType=="T3"){
    		$name = $this->getNameB3($contentName);
    	}
    	return $name;
    }
    ...
    function getNameBJPK($contentName){
    	...
    	if($betInfo[1]=="LOCATE"){//每球定位
    		...
    		if($selectBall == "1"){
    			if($betInfo[2]){
    				$name_bjpk = "冠军 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "冠军 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "2"){
    			if($betInfo[2]){
    				$name_bjpk = "亚军 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "亚军 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "3"){
    			if($betInfo[2]){
    				$name_bjpk = "季军 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "季军 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "4"){
    			if($betInfo[2]){
    				$name_bjpk = "第四名 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "第四名 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "5"){
    			if($betInfo[2]){
    				$name_bjpk = "第五名 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "第五名 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "6"){
    			if($betInfo[2]){
    				$name_bjpk = "第六名 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "第六名 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "7"){
    			if($betInfo[2]){
    				$name_bjpk = "第七名 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "第七名 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "8"){
    			if($betInfo[2]){
    				$name_bjpk = "第八名 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "第八名 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "9"){
    			if($betInfo[2]){
    				$name_bjpk = "第九名 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "第九名 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "10"){
    			if($betInfo[2]){
    				$name_bjpk = "第十名 ".$this->getCommonName($betInfo[2]);
    			}else{
    				$name_bjpk = "第十名 ".$this->getCommonName($betInfo[1]);
    			}
    		}
    ...
    function getNameGXSF($contentName){
    	...
    	if($betInfo[1]=="LOCATE"){//每球定位
    		...
    		if($selectBall == "1"){
    			if(count($betInfo)==4){
    				$name_gxsf = "正码一 ".$this->getCommonName($betInfo[1].":".$betInfo[3]);
    			}elseif($betInfo[2]){
    				$name_gxsf = "正码一 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gxsf = "正码一 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "2"){
    			if(count($betInfo)==4){
    				$name_gxsf = "正码二 ".$this->getCommonName($betInfo[1].":".$betInfo[3]);
    			}elseif($betInfo[2]){
    				$name_gxsf = "正码二 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gxsf = "正码二 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "3"){
    			if(count($betInfo)==4){
    				$name_gxsf = "正码三 ".$this->getCommonName($betInfo[1].":".$betInfo[3]);
    			}elseif($betInfo[2]){
    				$name_gxsf = "正码三 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gxsf = "正码三 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "4"){
    			if(count($betInfo)==4){
    				$name_gxsf = "正码四 ".$this->getCommonName($betInfo[1].":".$betInfo[3]);
    			}elseif($betInfo[2]){
    				$name_gxsf = "正码四 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gxsf = "正码四 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "S"){
    			if(count($betInfo)==4){
    				$name_gxsf = "特别号 ".$this->getCommonName($betInfo[1].":".$betInfo[3]);
    			}elseif($betInfo[2]){
    				$name_gxsf = "特别号 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gxsf = "特别号 ".$this->getCommonName($betInfo[1]);
    			}
    		}
    	}
    ...
    function getNameGDSF($contentName){
    	...
    	if($betInfo[1]=="LOCATE"){//每球定位
    		...
    		if($selectBall == "1"){
    			if($betInfo[2]){
    				$name_gdsf = "第一球 ".$this->$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gdsf = "第一球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "2"){
    			if($betInfo[2]){
    				$name_gdsf = "第二球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gdsf = "第二球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "3"){
    			if($betInfo[2]){
    				$name_gdsf = "第三球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gdsf = "第三球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "4"){
    			if($betInfo[2]){
    				$name_gdsf = "第四球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gdsf = "第四球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "5"){
    			if($betInfo[2]){
    				$name_gdsf = "第五球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gdsf = "第五球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "6"){
    			if($betInfo[2]){
    				$name_gdsf = "第六球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gdsf = "第六球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "7"){
    			if($betInfo[2]){
    				$name_gdsf = "第七球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gdsf = "第七球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "S"){
    			if($betInfo[2]){
    				$name_gdsf = "第八球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_gdsf = "第八球 ".$this->getCommonName($betInfo[1]);
    			}
    		}
    ...
    function getNameTJSF($contentName){
    	...
    	if($betInfo[1]=="LOCATE"){//每球定位
    		...
    		if($selectBall == "1"){
    			if($betInfo[2]){
    				$name_tjsf = "第一球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_tjsf = "第一球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "2"){
    			if($betInfo[2]){
    				$name_tjsf = "第二球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_tjsf = "第二球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "3"){
    			if($betInfo[2]){
    				$name_tjsf = "第三球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_tjsf = "第三球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "4"){
    			if($betInfo[2]){
    				$name_tjsf = "第四球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_tjsf = "第四球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "5"){
    			if($betInfo[2]){
    				$name_tjsf = "第五球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_tjsf = "第五球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "6"){
    			if($betInfo[2]){
    				$name_tjsf = "第六球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_tjsf = "第六球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "7"){
    			if($betInfo[2]){
    				$name_tjsf = "第七球 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_tjsf = "第七球 ".$this->getCommonName($betInfo[1]);
    			}
    		}elseif($selectBall == "S"){
    			if($betInfo[2]){
    				$name_tjsf = "特别号 ".$this->getCommonName($betInfo[1].":".$betInfo[2]);
    			}else{
    				$name_tjsf = "特别号 ".$this->getCommonName($betInfo[1]);
    			}
    		}
    		if($contentName == "1:S:DRAGON"){
    			$name_tjsf = "龙";
    ...
    function getNameB5($contentName){
    	$name_b5 = $this->get535NameByCode($contentName);
    	return $name_b5;
    }
    function getNameB3($contentName){
    	$name_b3 = $this->getOeouNameByCode($contentName);
    	if($name_b3 == $contentName){
    		if(strpos($contentName,"*")!==false){
    			...
    			if($betInfo[2]){
    				$name_b3 = $this->getOeouNameByCode($betInfo[0])."*".$this->getOeouNameByCode($betInfo[1])."*".$this->getOeouNameByCode($betInfo[2]);
    			}else{
    				$name_b3 = $this->getOeouNameByCode($betInfo[0])."*".$this->getOeouNameByCode($betInfo[1]);
    			}
    		}
    	}
    	return $name_b3;
