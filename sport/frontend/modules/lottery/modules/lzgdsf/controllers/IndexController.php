<?php
namespace app\modules\lottery\modules\lzgdsf\controllers;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='GDSF';
	const CONST_LOTTERY_NAME='广东十分彩';
	...
	public function actionIndex() {
        $Lottery_set = WebClose::getWebClose('gdsf');
		$result = LotteryResultGdsf::getKJResult();
	 ...
	 * */
    public static function getGdsfInfo(){
        ...
        $KJResult=LotteryResultGdsf::getKJResult();
        if ($KJResult){
    ...
    try {
		...
		$orderlottery->rtype_str='快速-广东快乐十分';
		$orderlottery->rtype='771';
		...
		for($i = 0; $i < count ( $data ); $i ++) {
          ...
          error_log('IndexController.insert：'.$e->getTraceAsString().'online:'.$e->getLine().'，type：lzgdsf，time:'.date('Y-m-d h:i:s',time()).''."\r\n", 3, "error.log");
	...
	public function actionOrderList(){
		...
		$list=LotteryResultGdsf::getResultList(NULL,$start);
