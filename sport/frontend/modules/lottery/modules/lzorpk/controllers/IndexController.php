<?php
namespace app\modules\lottery\modules\lzorpk\controllers;
...
use app\modules\lottery\modules\lzorpk\util\BallUtil;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='ORPK';
	const CONST_LOTTERY_NAME='老PK拾';
	...
	public function actionIndex() {
        $Lottery_set = WebClose::getWebClose('orpk');
		$result = LotteryResultOrpk::getKJResult();
     ...
     * */
    public static function getorpkInfo(){
        ...
        $KJResult=LotteryResultOrpk::getKJResult();
        if ($KJResult){
        ...
        $frontinfo = array (
            ...
            'numbers' => $lastQishu,
            'hm' => $hm,
            'kaijiang' => $schedule["kaijiang_time"],
        );
        return $frontinfo;
    }
    /**
    ...
    try {
		...
		$orderlottery->rtype_str='快速-老PK拾';
		$orderlottery->rtype='765';
...
// 			$imgutil->CreateImage(self::CONST_LOTTERY_TYPE,self::CONST_LOTTERY_NAME,$qiuhao,$wanfa,$schedule['qishu'],$bet_money_one,$fs_money,$bet_rate,$datereg_sub);
          ...
          error_log('IndexController.insert：'.$e->getTraceAsString().'online:'.$e->getLine().'，type：lzorpk，time:'.date('Y-m-d h:i:s',time()).''."\r\n", 3, "error.log");
    ...
    public function GetScheduleInfo() {
        ...
        $isLateNight = false;
        if(date('H:i:s', $time) > '00:00:00' && date('H:i:s', $time) < '04:00:00'){		   //假設跨天
            $isLateNight = true;
        }
        ...
        $schedule['qishu']=date ( 'Ymd', $time_day ) . sprintf("%04d", $scheduleinfo ['qishu']);
    ...
    public function actionAjaxchanglong(){
    	...
    	$list=LotteryResultOrpk::getResultList(NULL,$start);
