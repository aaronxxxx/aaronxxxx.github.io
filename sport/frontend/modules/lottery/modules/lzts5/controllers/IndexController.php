<?php
namespace app\modules\lottery\modules\lzts5\controllers;
...
use app\modules\lottery\modules\lzts5\util\BallUtil;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='TS';
	const CONST_LOTTERY_NAME='腾讯分分彩';
	...
	public function actionIndex() {
        $Lottery_set = WebClose::getWebClose('ts');
		$result = LotteryResultTs::getKJResult();
     ...
     * */
    public static function getTs5Info(){
        ...
        $KJResult=LotteryResultTs::getKJResult();
        if ($KJResult){
    ...
    try {
		...
		$orderlottery->rtype_str='快速-腾讯分分彩';
		$orderlottery->rtype='764';
		...
		for($i = 0; $i < count ( $data ); $i ++) {
          ...
          error_log('IndexController.insert：'.$e->getTraceAsString().'online:'.$e->getLine().'，type：lzts5，time:'.date('Y-m-d h:i:s',time()).''."\r\n", 3, "error.log");
    ...
    public function GetScheduleInfo() {
    	...
    	$schedule['qishu']=date ( 'Ymd', $time ) . sprintf("%04d", $scheduleinfo ['qishu']);
	...
	public function actionOrderList(){
		...
		$list=LotteryResultTs::getResultList(NULL,$start);
		...
		$heper=new Luzhu($list,$which,45,5,$type,45);
		$return=$heper->cout();
