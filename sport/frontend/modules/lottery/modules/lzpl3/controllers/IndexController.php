<?php
namespace app\modules\lottery\modules\lzpl3\controllers;
...
use app\modules\lottery\modules\lzpl3\util\BallUtil;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='P3';
	const CONST_LOTTERY_NAME='排列三';
	...
	public function actionIndex() {
		...
		$result = LotteryResultP3::getKJResult();
     ...
     * */
    public static function getPl3Info(){
        ...
        $KJResult=LotteryResultP3::getKJResult();
        if ($KJResult){
    ...
    try {
		...
		$orderlottery->rtype_str='快速-排列三';
		$orderlottery->rtype='748';
		...
		for($i = 0; $i < count ( $data ); $i ++) {
          ...
          error_log('IndexController.insert：'.$e->getTraceAsString().'online:'.$e->getLine().'，type：lzpl3，time:'.date('Y-m-d h:i:s',time()).''."\r\n", 3, "error.log");
    ...
    public function GetScheduleInfo() {
        ...
        $jiaodui = WebClose::getWebClose('pl_jiaod');
        $Lottery_set['d3']['ktime']	= isset($jiaodui['kaijiang_time'])?date('Y-m-d',strtotime($jiaodui['kaijiang_time'])):'2017-01-01'; //default:2017-01-01
        $Lottery_set['d3']['knum']	=	isset($jiaodui['qishu'])?$jiaodui['qishu']:'2017001';//default:2017001
    	$lasttime = $Lottery_set ['d3'] ['ktime'] . ' ' . $times;
    	$thistime = date ( 'Y-m-d H:i:s', time () );
        $strto_time = strtotime($thistime) - strtotime($lasttime);
        $lost_days = round($strto_time/86400); //计算两个日期时间差
    	$qishu = $lost_days +$Lottery_set ['d3'] ['knum'];
    	if ((date ( 'H:i:s', time () ) <= '23:59:59') && ('21:00:00' <= date ( 'H:i:s', time () ))) {
    		$qishu = $qishu + 1;
    		$time = time () + 86400;
    	} else {
    		$time = time ();
    	...
    	$schedule['qishu']=$qishu;
	   	$schedule['fenpan_time']=  date ( 'Y-m-d', $time )  . ' 20:28:00';
    	$schedule['kaijiang_time']=date ( 'Y-m-d', $time )  . ' 20:58:00';
