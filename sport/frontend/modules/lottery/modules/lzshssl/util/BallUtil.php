<?php
namespace app\modules\lottery\modules\lzshssl\util;

class BallUtil {
	/**
	 * 算出开奖结果，单双，大小
	 *
	 * @param unknown $num
	 * @param unknown $type
	 * @return number string
	 */
	function f3D_Auto($num, $type) {
		$zh = $num [0] + $num [1] + $num [2];
		if ($type == 1) {
			return $zh;
		}
		if ($type == 2) {
			if (14 <= $zh) {
				return '总和大';
			}
			if ($zh <= 13) {
				return '总和小';
			}
		}
		if ($type == 3) {
			if (($zh % 2) == 0) {
				return '总和双';
			} else {
				return '总和单';
			}
		}
		if ($type == 4) {
			if ($num [2] < $num [0]) {
				return '龙';
			}
			if ($num [0] < $num [2]) {
				return '虎';
			}
			if ($num [0] == $num [2]) {
				return '和';
			}
		}
		if ($type == 5) {
			$n1 = $num [0];
			$n2 = $num [1];
			$n3 = $num [2];
			if ((($n1 == 0) || ($n2 == 0) || ($n3 == 0)) && (($n1 == 9) || ($n2 == 9) || ($n3 == 9))) {
				if ($n1 == 0) {
					$n1 = 10;
				}
				if ($n2 == 0) {
					$n2 = 10;
				}
				if ($n3 == 0) {
					$n3 = 10;
				}
			}
				
			if (($n1 == $n2) && ($n2 == $n3)) {
				return '豹子';
			} else {
				if (($n1 == $n2) || ($n1 == $n3) || ($n2 == $n3)) {
					return '对子';
				} else {
					if ((($n1 == 10) || ($n2 == 10) || ($n3 == 10)) && (($n1 == 9) || ($n2 == 9) || ($n3 == 9)) && (($n1 == 1) || ($n2 == 1) || ($n3 == 1))) {
						return '顺子';
					} else {
						if (((abs ( $n1 - $n2 ) == 1) && (abs ( $n2 - $n3 ) == 1)) || ((abs ( $n1 - $n2 ) == 2) && (abs ( $n1 - $n3 ) == 1) && (abs ( $n2 - $n3 ) == 1)) || ((abs ( $n1 - $n2 ) == 1) && (abs ( $n1 - $n3 ) == 1))) {
							return '顺子';
						} else {
							if ((abs ( $n1 - $n2 ) == 1) || (abs ( $n1 - $n3 ) == 1) || (abs ( $n2 - $n3 ) == 1)) {
								return '半顺';
							} else {
								return '杂六';
							}
						}
					}
				}
			}
		}
		if ($type == 6) {
			return $this->get_max ( $num [0], $num [1], $num [2] ) - $this->get_min ( $num [0], $num [1], $num [2] );
		}
	}
	function get_max($a, $b, $c) {
		return $b < $a ? ($c < $a ? $a : $c) : ($c < $b ? $b : $c);
	}
	function get_min($a, $b, $c) {
		return $a < $b ? ($a < $c ? $a : $c) : ($b < $c ? $b : $c);
	}
	
	
	//读取第几球
	function getdid ($type) {
		$r = '';
		switch ($type) {
			case 1 : $r = '第一球'; break;
			case 2 : $r = '第二球'; break;
			case 3 : $r = '第三球'; break;
			case 4 : $r = '总和龙虎和'; break;
			case 5 : $r = '三连'; break;
			case 6 : $r = '跨度'; break;
		}
		return $r;
	}
	//读取玩法
	function getwan ($type) {
		$r = '';
		switch ($type) {
			case 1 : $r = '0'; break;
			case 2 : $r = '1'; break;
			case 3 : $r = '2'; break;
			case 4 : $r = '3'; break;
			case 5 : $r = '4'; break;
			case 6 : $r = '5'; break;
			case 7 : $r = '6'; break;
			case 8 : $r = '7'; break;
			case 9 : $r = '8'; break;
			case 10 : $r = '9'; break;
			case 11 : $r = '大'; break;
			case 12 : $r = '小'; break;
			case 13 : $r = '单'; break;
			case 14 : $r = '双'; break;
		}
		return $r;
	}
	//读取玩法
	function getwan6 ($type) {
		$r = '';
		switch ($type) {
			case 1 : $r = '总和大'; break;
			case 2 : $r = '总和小'; break;
			case 3 : $r = '总和单'; break;
			case 4 : $r = '总和双'; break;
			case 5 : $r = '龙'; break;
			case 6 : $r = '虎'; break;
			case 7 : $r = '和'; break;
		}
		return $r;
	}
	//读取玩法
	function getwan789 ($type) {
		$r = '';
		switch ($type) {
			case 1 : $r = '豹子'; break;
			case 2 : $r = '顺子'; break;
			case 3 : $r = '对子'; break;
	
			case 4 : $r = '半顺'; break;
			case 5 : $r = '杂六'; break;
		}
		return $r;
	}
	
	//读取跨度
	function getwanK ($type) {
		$r = '';
		switch ($type) {
			case 1 : $r = '0'; break;
			case 2 : $r = '1'; break;
			case 3 : $r = '2'; break;
			case 4 : $r = '3'; break;
			case 5 : $r = '4'; break;
			case 6 : $r = '5'; break;
			case 7 : $r = '6'; break;
			case 8 : $r = '7'; break;
			case 9 : $r = '8'; break;
			case 10 : $r = '9'; break;
		}
		return $r;
	}
	
}