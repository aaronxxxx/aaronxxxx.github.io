<?php
namespace app\modules\lottery\modules\lztjssc\controllers;
...
use app\modules\lottery\modules\lztjssc\util\BallUtil;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='TJ';
	const CONST_LOTTERY_NAME='极速时时彩';
	...
	public function actionIndex() {
        $Lottery_set = WebClose::getWebClose('tj');
		$result = LotteryResultTj::getKJResult();
     ...
     * */
    public static function getTjsscInfo(){
        ...
        $KJResult=LotteryResultTj::getKJResult();
        if ($KJResult){
    ...
    try {
		...
		$orderlottery->rtype_str='快速-极速时时彩';
		$orderlottery->rtype='721';
		...
		for($i = 0; $i < count ( $data ); $i ++) {
          ...
          error_log('IndexController.insert：'.$e->getTraceAsString().'online:'.$e->getLine().'，type：lztjssc，time:'.date('Y-m-d h:i:s',time()).''."\r\n", 3, "error.log");
	...
	public function actionOrderList(){
		...
		$list=LotteryResultTj::getResultList(NULL,$start);
