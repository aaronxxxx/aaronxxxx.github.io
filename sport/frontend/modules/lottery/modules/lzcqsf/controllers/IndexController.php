<?php
namespace app\modules\lottery\modules\lzcqsf\controllers;
...
use app\modules\lottery\modules\lzcqsf\util\BallUtil;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='CQSF';
	const CONST_LOTTERY_NAME='重庆十分彩';
	...
	public function actionIndex() {
		$result = LotteryResultCqsf::getKJResult();
     ...
     * */
    public static function getCqsfcInfo(){
        ...
        $KJResult=LotteryResultCqsf::getKJResult();     //查找最后一期
        if ($KJResult)
        {
	...
	public function actionGetKJResult() {
		$KJResult=LotteryResultCqsf::getKJResult();     //查找最后一期 按期数倒序
		if ($KJResult)
		{
	...
	public function actionPrepareOrder() {
        $data = $this->getParam('data','');  //{ball_9_1: "11"}
        $data=DataValid::object_array(json_decode($data));      //Array ( [ball_9_1] => 11 )
		$ret = $this->data_valid ( $data );     //验证数据:正确没返回 错误返回{msg：错误信息！ status：false}
		if ($ret) {
			return $ret;
		}
		$ballutil=new BallUtil();
		$oddslist=$this->GetOddsList();  //获取赔率
		...
		for($i = 0; $i < count ( $data ); $i ++) {
			...
			$sumbetball ++;
			$qiu = explode ( '_', $names [$i] );    //切割数据
			$odd = $oddslist ['ball'] [$qiu [1]] [$qiu [2]];
			$qiuhao = $ballutil->getdid($qiu [1]);      //获取投注内容 第一球、第二球、、、
			if ($qiu [1] == 9) {
				$wanfa = $ballutil->getwan9 ($qiu [2]);     //总和大、总和小、、、
			} else {
				$wanfa = $ballutil->getwan ($qiu [2]);      //投注内容：1、2、3、4、大、小、中、、、
			}
	...
	public function actionInsertOrder() {
		$data = $this->getParam('data',''); //获取数据 {ball_9_1: "11"}
		$data=DataValid::object_array(json_decode($data)); //对象转array :Array ( [ball_9_1] => 11 )
		$ret = $this->data_valid ( $data );     // 效验数据
		if ($ret) {
			return $ret;
		}
    ...
    try {
        $userid=Yii::$app->session->get('uid');     //获取sessiong
		$oddslist=$this->GetOddsList();     //获取赔率
		$schedule=$this->GetScheduleInfo();     //获取开奖信息，封盘、开奖、期数
		$currenttime=date("Y-m-d H:i:s",time());
        /*存入订单表*/
		$orderlottery=new OrderLottery();
		$orderlottery->user_id=$userid;
		$orderlottery->Gtype=self::CONST_LOTTERY_TYPE;
		$orderlottery->rtype_str='快速-重庆快乐十分';
		$orderlottery->rtype='791';
		...
		$userinfo=UserList::getUserInfo($userid);       //获取用户信息
        $usermoney=$userinfo['money'];      //提取用户金额
        //$money_usermoney = MoneyLog::GetLastMoney($userid);     //获取money_log 最后金额
		$balance=$usermoney-DataValid::sum_bet_money ( $data );     //用户金额减去投注金额
    ...
    public function GetOddsList() {
    	...
    	$odds=OddsLottery::getOdds(self::CONST_LOTTERY_NAME, $lottery_subtype);  //取得赔率
    	// 组装赔率  前面调用已经固定了，但是这边整体还是可以微调的
        for($s = 0; $s < 10 ; $s ++) {
    		for($i = 1; $i < 25; $i ++) {
                if($s==8){
     ...
     * return（Array(
        [qishu] => 20170213014
        [kaipan_time] => 2017-02-13 02:03:00
        [fenpan_time] => 2017-02-13 10:01:00
        [kaijiang_time] => 2017-02-13 10:03:00
    )）
     */
    public function GetScheduleInfo() {
        ...
        $timeKaipan = time();
        $time = time ();
        if (date ( 'H:i:s', time () ) >= '23:53:00' || date ( 'H:i:s', time () ) <= '00:03:00') {
            $scheduleinfo = $firstSchedule;
            if(date ( 'H:i:s', time () ) >= '23:53:00'){
                $time = time () + 86400;
            }
            if(date ( 'H:i:s', time () ) <= '00:03:00'){
                $timeKaipan = time() - 86400;
            }
        }else {
    		$scheduleinfo = LotterySchedule::getNewSchedule ( self::CONST_LOTTERY_NAME );
    	}
        $schedule['qishu']=date ( 'Ymd', $time ) . $scheduleinfo ['qishu'];
    	$schedule['kaipan_time']=date ( 'Y-m-d', $timeKaipan ) . ' ' . $scheduleinfo ['kaipan_time'];
     ...
     * @param unknown $data
     * 只是验证验证用户组的最大金额 - 当期最大金额还需要重写
     */
    function system_bet_limit( $data) {
    	$bet_money_total = DataValid::sum_bet_money ( $data );      //获取投注总额
    	$schedule=$this->GetScheduleInfo();
    	$qishu=$schedule["qishu"];
        $userid=Yii::$app->session[Yii::$app->params['S_USER_ID']];
    	$ugul=UserGroup::getUserGroupInfo($userid);  //获取用户组信息 可以优化：把lottery_max_bet当成参数 带进入
    	$maxMoney=$ugul[strtolower ( self::CONST_LOTTERY_TYPE ) . '_max_bet'];
    	//$max_money_already = OrderLottery::getSumBetMoney ( self::CONST_LOTTERY_TYPE, $qishu ,$userid);
    	// 校验该用户最大的投注金额
    	//if ((0 < $maxMoney) && ($maxMoney < ($max_money_already + $bet_money_total))) {
        if ((0 < $maxMoney) && ($maxMoney <  $bet_money_total)) {
    		return false;
    	}
    	return true;
    }
	...
	public function actionOrderList(){
		...
		$list=LotteryResultCqsf::getResultList(NULL,$start);
		$list=array_slice($list,0,120);
		if($type ==1){
            foreach ($list as $row=>$v){
                $guanyanhe = $v['ball_1'] + $v['ball_2'] + $v['ball_3'] + $v['ball_4'] + $v['ball_5'] + $v['ball_6'] + $v['ball_7'] + $v['ball_8'];
                if($guanyanhe == 84){
                    $longhuhe = 2;
                }else{
                    if($guanyanhe < 84){
	...
	public function actionLonghu(){
		...
		for($b_a=0;$b_a<count($list);$b_a++){
			if($list[$b_a]['ball_1'] > $list[$b_a]['ball_8']){
				$longhuhe = 0;
			}else if($list[$b_a]['ball_1'] < $list[$b_a]['ball_8']){
				$longhuhe = 1;
			}else{
				$longhuhe = 2;
			}
	...
	public function actionAjaxchanglong(){
        ...
        $changlong = [];
        $arr = ['一',"二","三","四","五","六","七","八"];
        $changlong['总和 - 单'] = $liangmian->count_single($list,8);
        $changlong['总和 - 双'] = $liangmian->count_double($list,8);
        $changlong['总和 - 大'] = $liangmian->count_big($list,8,84);
        $changlong['总和 - 小'] = $liangmian->count_small($list,8,84);
        for($i=1;$i<=8;$i++){
        ...
        $changlong['龙虎 - 龙'] = $liangmian->long($list,1,8);
        $changlong['龙虎 - 虎'] = $liangmian->hu($list,1,8);
        $changlong['龙虎 - 合'] = $liangmian->he($list,1,8);
        arsort($changlong);
	...
	public function actionLuzhutu(){
		...
		$heper=new HelperFun($list,$which,20,8,$type);
		$return=$heper->cout();
