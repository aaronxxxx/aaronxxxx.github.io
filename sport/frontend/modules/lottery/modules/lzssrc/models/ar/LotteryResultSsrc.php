...
namespace app\modules\lottery\modules\lzssrc\models\ar;
...
class LotteryResultSsrc extends \yii\db\ActiveRecord
{
	...
	{
        $lastResult = "select * from lottery_result_ssrc where datetime <= NOW() order by datetime desc limit 1";
        $rsarr = self::findBySql($lastResult)->asArray()->one();
        return $rsarr;
	}
    public static function getKJResultForTableNum($table,$num)
	{
        $lastResult = "select * from ".$table." where datetime <= NOW() order by datetime desc limit ".$num;
        $rsarr = self::findBySql($lastResult)->asArray()->all();
        return $rsarr;
	}
	public static function getResultList($qishu_query=null,$query_time=null)
	{
		if($qishu_query==null){
            ...
            $rslist=self::find()->where('DATE_FORMAT(datetime,"%Y-%m-%d") = :query_time', [':query_time' => $query_time])->andWhere('datetime <= NOW()')->orderBy(['qishu' => SORT_DESC,])
			->asArray()
			->all();
			return $rslist;
		}else{
