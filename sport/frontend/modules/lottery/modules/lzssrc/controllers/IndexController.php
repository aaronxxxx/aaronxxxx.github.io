<?php
namespace app\modules\lottery\modules\lzssrc\controllers;
...
use app\modules\lottery\modules\lzssrc\util\BallUtil;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='SSRC';
	const CONST_LOTTERY_NAME='极速赛车';
	...
	public function actionIndex() {
        $Lottery_set = WebClose::getWebClose('ssrc');
        $result = LotteryResultSsrc::getKJResult();
     ...
     * */
    public static function getSSrcInfo(){
        ...
        $KJResult=LotteryResultSsrc::getKJResult();
        if ($KJResult){
    ...
    try {
		...
		$orderlottery->rtype_str='快速-极速赛车';
		$orderlottery->rtype='762';
...
// 			$imgutil->CreateImage(self::CONST_LOTTERY_TYPE,self::CONST_LOTTERY_NAME,$qiuhao,$wanfa,$schedule['qishu'],$bet_money_one,$fs_money,$bet_rate,$datereg_sub);
          ...
          error_log('IndexController.insert：'.$e->getTraceAsString().'online:'.$e->getLine().'，type：lzssrc，time:'.date('Y-m-d h:i:s',time()).''."\r\n", 3, "error.log");
    ...
    public function actionAjaxchanglong(){
    	...
    	$list=LotteryResultSsrc::getResultList(NULL,$start);
