<?php
namespace app\modules\lottery\modules\lzpk10\controllers;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='BJPK';
	const CONST_LOTTERY_NAME='北京PK拾';
	...
	public function actionIndex() {
        $Lottery_set = WebClose::getWebClose('pk10');
		$result = LotteryResultBjpk::getKJResult();
     ...
     * */
    public static function getBjpk10Info(){
        ...
        $KJResult=LotteryResultBjpk::getKJResult();
        if ($KJResult){
    ...
    try {
		...
		$orderlottery->rtype_str='快速-北京PK拾';
		$orderlottery->rtype='761';
...
// 			$imgutil->CreateImage(self::CONST_LOTTERY_TYPE,self::CONST_LOTTERY_NAME,$qiuhao,$wanfa,$schedule['qishu'],$bet_money_one,$fs_money,$bet_rate,$datereg_sub);
          ...
          error_log('IndexController.insert：'.$e->getTraceAsString().'online:'.$e->getLine().'，type：lzpk10，time:'.date('Y-m-d h:i:s',time()).''."\r\n", 3, "error.log");
    ...
    public function GetScheduleInfo() {
        ...
        $jiaodui = WebClose::getWebClose('bjpk_jia');
        $Lottery_set['pk10']['ktime']	= isset($jiaodui['kaijiang_time'])?date('Y-m-d',strtotime($jiaodui['kaijiang_time'])):'2017-01-01'; //default:2017-01-01
        $Lottery_set['pk10']['knum']	= isset($jiaodui['qishu'])?$jiaodui['qishu']:'595162';//default:595162
    	$lasttime = $Lottery_set ['pk10'] ['ktime'] . ' ' . $times;
    	...
    	$schedule['qishu']=(($lost_days * 44) + $Lottery_set['pk10']['knum'] + $scheduleinfo['qishu']) - 1;
    ...
    public function actionAjaxchanglong(){
    	...
    	$list=LotteryResultBjpk::getResultList(NULL,$start);
