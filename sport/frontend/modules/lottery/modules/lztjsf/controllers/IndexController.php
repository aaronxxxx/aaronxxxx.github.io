<?php
namespace app\modules\lottery\modules\lztjsf\controllers;
...
use app\modules\lottery\modules\lztjsf\util\BallUtil;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='TJSF';
	const CONST_LOTTERY_NAME='天津十分彩';
	...
	public function actionIndex() {
        $Lottery_set = WebClose::getWebClose('tjsf');
		$result = LotteryResultTjsf::getKJResult();
     ...
     * */
    public static function getTjsfcInfo(){
        ...
        $KJResult=LotteryResultTjsf::getKJResult();
        if ($KJResult){
    ...
    try {
		...
		$orderlottery->rtype_str='快速-天津快乐十分';
		$orderlottery->rtype='781';
		...
		for($i = 0; $i < count ( $data ); $i ++) {
          ...
          error_log('IndexController.insert：'.$e->getTraceAsString().'online:'.$e->getLine().'，type：lztjsf，time:'.date('Y-m-d h:i:s',time()).''."\r\n", 3, "error.log");
	...
	public function actionOrderList(){
		...
		$list=LotteryResultTjsf::getResultList(NULL,$start);
