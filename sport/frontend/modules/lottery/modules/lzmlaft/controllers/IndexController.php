<?php
namespace app\modules\lottery\modules\lzmlaft\controllers;
...
use app\modules\lottery\modules\lzmlaft\util\BallUtil;
...
class IndexController extends BaseController {
	const CONST_LOTTERY_TYPE='MLAFT';
	const CONST_LOTTERY_NAME='幸运飞艇';
	...
	public function actionIndex() {
        $Lottery_set = WebClose::getWebClose('mlaft');
		$result = LotteryResultMlaft::getKJResult();
     ...
     * */
    public static function getMlaftInfo(){
        ...
        $KJResult=LotteryResultMlaft::getKJResult();
        if ($KJResult){
    ...
    try {
		...
		$orderlottery->rtype_str='快速-幸运飞艇';
		$orderlottery->rtype='763';
...
// 			$imgutil->CreateImage(self::CONST_LOTTERY_TYPE,self::CONST_LOTTERY_NAME,$qiuhao,$wanfa,$schedule['qishu'],$bet_money_one,$fs_money,$bet_rate,$datereg_sub);
		}
          ...
          error_log('IndexController.insert：'.$e->getTraceAsString().'online:'.$e->getLine().'，type：lzmlaft，time:'.date('Y-m-d h:i:s',time()).''."\r\n", 3, "error.log");
    ...
    public function GetScheduleInfo() {
        $time = time();    //test : strtotime('2019-02-21 00:40:00');
        $isLateNight = false;
        if(date('H:i:s', $time) >= '00:00:00' && date('H:i:s', $time) < '04:05:00'){		   //假設跨天
            $isLateNight = true;
        }
        $isLateNight == true ? $time_day = $time - 86400 : $time_day = $time;                                   //則減去一天時間
        $scheduleinfo = LotterySchedule::getNewScheduleForMlaft ( self::CONST_LOTTERY_NAME );       //修正幸運飛艇的取得期數會有誇天問題需要分開寫
        $schedule['qishu']=date ( 'Ymd', $time_day ) . sprintf("%03d", $scheduleinfo ['qishu']);
        if($scheduleinfo ['qishu'] == '132')        //假設跨天第一期
        {

            if(date ( 'H:i:s', $time ) >= '23:59:00') {//23:59:00~ 00:00:00
                $schedule['kaipan_time']=date ( 'Y-m-d', $time ) . ' ' . $scheduleinfo ['kaipan_time'];
                $schedule['fenpan_time']=date ( 'Y-m-d', $time + 86400 ) . ' ' . $scheduleinfo ['fenpan_time'];
                $schedule['kaijiang_time']=date ( 'Y-m-d', $time + 86400 ) . ' ' . $scheduleinfo ['kaijiang_time'];
                // var_dump(date('Y-m-d H:i:s', $time));
                // var_dump($schedule['kaipan_time']);
                // var_dump($schedule['fenpan_time']);
                // var_dump($schedule['kaijiang_time']);
            }else{                                      //00:00:00~00:04:00
                $schedule['kaipan_time']=date ( 'Y-m-d', $time - 86400 ) . ' ' . $scheduleinfo ['kaipan_time'];
    ...
    public function actionAjaxchanglong(){
    	...
    	$list=LotteryResultMlaft::getResultList(NULL,$start);
        $liangmian = new LiangMian();
        $changlong = [];
        $arr = ['冠军','亚军','第三名','第四名','第五名','第六名','第七名','第八名','第九名','第十名'];
        $changlong['冠亚军和 - 大'] = $liangmian->pk10GuanYaDa($list);
        $changlong['冠亚军和 - 小'] = $liangmian->pk10GuanYaXiao($list);
        $changlong['冠亚军和 - 单'] = $liangmian->pk10GuanYaDan($list);
        $changlong['冠亚军和 - 双'] = $liangmian->pk10GuanYaShuang($list);
        for($i=1;$i<=10;$i++){
            $changlong[$arr[$i-1].' - 大'] = $liangmian->ball_big($list,$i,5);
            $changlong[$arr[$i-1].' - 小'] = $liangmian->ball_small($list,$i,6);
            $changlong[$arr[$i-1].' - 单'] = $liangmian->ball_single($list,$i);
            $changlong[$arr[$i-1].' - 双'] = $liangmian->ball_double($list,$i);
        }
        for($i=1;$i<=5;$i++){
            $changlong[$arr[$i-1].' - 龙'] = $liangmian->pk10Long($list,$i);
            $changlong[$arr[$i-1].' - 虎'] = $liangmian->pk10Hu($list,$i);
        }
        arsort($changlong);
