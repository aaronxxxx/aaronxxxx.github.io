    ...
    <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:2px;" bgcolor="#798EB9">
        ...
        foreach($rslist as $key=>$rows){
            ...
            <tr   class="R_tr" align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                ...
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_0/<?=$ballutil->BuLing($rows['ball_1'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_0/<?=$ballutil->BuLing($rows['ball_2'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_0/<?=$ballutil->BuLing($rows['ball_3'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_0/<?=$ballutil->BuLing($rows['ball_4'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_0/<?=$ballutil->BuLing($rows['ball_5'])?>.png"></td>
                <td><?=$ballutil->gd11x5_Auto($hm,1)?> / <?=($ballutil->gd11x5_Auto($hm,2)=="总和大"?"<span style=\"color: red;\">".$ballutil->gd11x5_Auto($hm,2)."</span>":$ballutil->gd11x5_Auto($hm,2))?> / <?=($ballutil->gd11x5_Auto($hm,3)=="总和双"?"<span style=\"color: red;\">".$ballutil->gd11x5_Auto($hm,3)."</span>":$ballutil->gd11x5_Auto($hm,3))?></td>
                <td><?=($ballutil->gd11x5_Auto($hm,4)=="龙"?"<span style=\"color: red;\">".$ballutil->gd11x5_Auto($hm,4)."</span>":$ballutil->gd11x5_Auto($hm,4))?></td>
                <td><?=$ballutil->gd11x5_Auto($hm,5)?> / <?=$ballutil->gd11x5_Auto($hm,6)?> / <?=$ballutil->gd11x5_Auto($hm,7)?></td>
            </tr>
        <?php
        }
        if($hasRow=="false"){
