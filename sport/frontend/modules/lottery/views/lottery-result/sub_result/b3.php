    ...
    <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:2px;" bgcolor="#798EB9">
        ...
        foreach($rslist as $key=>$rows){
            ...
            <tr class="R_tr" align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                ...
                <td><?=$ballutil->f3D_Auto($hm,1)?> / <?=($ballutil->f3D_Auto($hm,2)=="总和大"?"<span style=\"color: red;\">".$ballutil->f3D_Auto($hm,2)."</span>":$ballutil->f3D_Auto($hm,2))?> / <?=($ballutil->f3D_Auto($hm,3)=="总和双"?"<span style=\"color: red;\">".$ballutil->f3D_Auto($hm,3)."</span>":$ballutil->f3D_Auto($hm,3))?></td>
                <td><?=($ballutil->f3D_Auto($hm,4)=="龙"?"<span style=\"color: red;\">".$ballutil->f3D_Auto($hm,4)."</span>":$ballutil->f3D_Auto($hm,4))?></td>
                <td><?=$ballutil->f3D_Auto($hm,5)?></td>
                <td><?=$ballutil->f3D_Auto($hm,6)?></td>
            </tr>
        <?php
        }
        if($hasRow=="false"){
