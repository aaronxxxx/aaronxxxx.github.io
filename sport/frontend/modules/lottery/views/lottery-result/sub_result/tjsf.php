    ...
    <table width="100%" border="0" cellpadding="5" cellspacing="1" class="font12" style="margin-top:2px;" bgcolor="#798EB9">
        ...
        foreach($rslist as $key=>$rows){
            ...
            <tr   class="R_tr" align="center" onMouseOver="this.style.backgroundColor='<?=$over?>'" onMouseOut="this.style.backgroundColor='<?=$out?>'" style="background-color:<?=$color?>; line-height:20px;">
                ...
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_1/<?=$ballutil->BuLing($rows['ball_1'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_1/<?=$ballutil->BuLing($rows['ball_2'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_1/<?=$ballutil->BuLing($rows['ball_3'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_1/<?=$ballutil->BuLing($rows['ball_4'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_1/<?=$ballutil->BuLing($rows['ball_5'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_1/<?=$ballutil->BuLing($rows['ball_6'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_1/<?=$ballutil->BuLing($rows['ball_7'])?>.png"></td>
                <td align="center" valign="middle"><img src="<?=$imgUrl?>/Ball_1/<?=$ballutil->BuLing($rows['ball_8'])?>.png"></td>
                <td><?=$ballutil->G10_Auto($hm,1)?> / <?=($ballutil->G10_Auto($hm,2)=="总和大"?"<span style=\"color: red;\">".$ballutil->G10_Auto($hm,2)."</span>":$ballutil->G10_Auto($hm,2))?> / <?=($ballutil->G10_Auto($hm,3)=="总和双"?"<span style=\"color: red;\">".$ballutil->G10_Auto($hm,3)."</span>":$ballutil->G10_Auto($hm,3))?> / <?=($ballutil->G10_Auto($hm,4)=="总和尾大"?"<span style=\"color: red;\">".$ballutil->G10_Auto($hm,4)."</span>":$ballutil->G10_Auto($hm,4))?></td>
                <td><?=($ballutil->G10_Auto($hm,5)=="龙"?"<span style=\"color: red;\">".$ballutil->G10_Auto($hm,5)."</span>":$ballutil->G10_Auto($hm,5))?></td>
            </tr>
        <?php
        }
        if($hasRow=="false"){
