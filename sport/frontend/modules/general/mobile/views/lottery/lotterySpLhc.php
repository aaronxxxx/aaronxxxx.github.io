    ...
    <div class="charge report">
        ...
        <div class="datepicker">
            <ul id="" class="d-flex justify-content-between fastDate mb-3">
                <li class="item">
                    <a href="/?r=mobile/lottery/lottery-splhc&time=<?= date('Y-m-d',strtotime('-0 day'))?>&type=LT">
                        <p>今日</p>
                    </a>
                </li>
                <li class="item">
                    <a href="/?r=mobile/lottery/lottery-splhc&time=<?= date('Y-m-d',strtotime('-1 day'))?>&type=LT">
                        <p>昨日</p>
                    </a>
                </li>
                <!-- <li class="item">
        ...
        <div id="tabinner" class="tabinner">
            ...
            <h4 class="dateTitle text-center"><?php echo isset($_GET['time']) ? $_GET['time']:date('Y-m-d')?></h4>

            <table class="MMain lhcjilu" cellspacing="0">
                <thead>
                    <tr class="title_tr">
                        <th>期号</th>
                        <th>玩法</th>
                ...
                <?php
                    if ($arr1['result'] != 1) {
                        foreach ($arr2 as $key => $rows) {
                            ...
                            <tr >
                                <td><?= $rows["qishu"] ?></td>
                                <td ><?= $rows["rtype_str_sub"] ?></td>
                                <td ><?= $rows["number"] ?></td>
                                <td ><?= $rows["bet_money_one"] ?></td>
                                <td ><?= $rows["fs"] ?></td>
                                <td ><?= $rows["bet_rate"] ?></td>
                                <td ><?= $rows["money_result"] ?></td>
                                <td><?= $rows["status_result"] ?></td>
                            </tr>
                            <?php
                        }
                    } else {
