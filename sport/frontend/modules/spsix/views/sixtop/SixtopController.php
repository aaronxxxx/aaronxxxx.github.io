...
namespace frontend\modules\spsix\controllers;

use app\modules\spsix\helpers\Zodiac;
use Yii;
use yii\web\Controller;
use frontend\modules\spsix\models\SpsixLotteryOrder;
use frontend\modules\spsix\models\SpsixLotteryOdds;
use frontend\modules\spsix\models\SpsixLotterySchedule;
use frontend\modules\spsix\models\OrderLottery;
use frontend\modules\spsix\models\LotteryResultSplhc;
use frontend\modules\spsix\models\UserList;
use frontend\modules\spsix\models\SysAnnouncement;
use frontend\modules\spsix\models\CommonFc\CommonFc;
...
class SixtopController extends Controller {
    ...
    public function actionXiaZhu() {
        ...
        $date = date("Y-m-d");
        $lhc_today_result = SpsixLotteryOrder::getOneDayOrder($userid, $date);
...
//        if($type=='LT'){
//            $arr = SpsixLotteryOrder::getId($day, $where);
//        }else{
...
//        if($bid){
//            if($type=='LT'){
//                $rows = SpsixLotteryOrder::getAllNews($bid);
//            }else{
...
//
    ...
    public function actionHistory() {
        ...
        $lhc_today_result = SpsixLotteryOrder::getOneDayOrder($userid, date("Y-m-d"));
        $lhc_day1_result = SpsixLotteryOrder::getOneDayOrder($userid, date('Y-m-d', strtotime('-1 day')));
        $lhc_day2_result = SpsixLotteryOrder::getOneDayOrder($userid, date('Y-m-d', strtotime('-2 day')));
        $lhc_day3_result = SpsixLotteryOrder::getOneDayOrder($userid, date('Y-m-d', strtotime('-3 day')));
        $lhc_day4_result = SpsixLotteryOrder::getOneDayOrder($userid, date('Y-m-d', strtotime('-4 day')));
        $lhc_day5_result = SpsixLotteryOrder::getOneDayOrder($userid, date('Y-m-d', strtotime('-5 day')));
        $lhc_day6_result = SpsixLotteryOrder::getOneDayOrder($userid, date('Y-m-d', strtotime('-6 day')));

        $lhc_today_win = SpsixLotteryOrder::getOneDayTotalWin($userid, date("Y-m-d"));
        $lhc_day1_win = SpsixLotteryOrder::getOneDayTotalWin($userid, date('Y-m-d', strtotime('-1 day')));
        $lhc_day2_win = SpsixLotteryOrder::getOneDayTotalWin($userid, date('Y-m-d', strtotime('-2 day')));
        $lhc_day3_win = SpsixLotteryOrder::getOneDayTotalWin($userid, date('Y-m-d', strtotime('-3 day')));
        $lhc_day4_win = SpsixLotteryOrder::getOneDayTotalWin($userid, date('Y-m-d', strtotime('-4 day')));
        $lhc_day5_win = SpsixLotteryOrder::getOneDayTotalWin($userid, date('Y-m-d', strtotime('-5 day')));
        $lhc_day6_win = SpsixLotteryOrder::getOneDayTotalWin($userid, date('Y-m-d', strtotime('-6 day')));
    ...
    public function actionDatecount(){
        ...
        $lhc_result = SpsixLotteryOrder::getOneDayOrder($userid,$date_select);
        $lhc_win = SpsixLotteryOrder::getOneDayTotalWin($userid,$date_select);
    ...
    public function actionkaijiang() {
        ...
        $arr = LotteryResultSplhc::getSixResult($qishu_query,'',$time);
        $hasRow = 'false';
