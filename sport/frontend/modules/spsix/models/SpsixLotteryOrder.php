...
/**
 * 极速六合彩表订单操作
 * SpsixLotteryOrder is the model behind the six_lottery_order.
 */
class SpsixLotteryOrder extends ActiveRecord {
    public static function tableName()
    {
        return '{{spsix_lottery_order}}';
    }
    ...
    static public function DelSixOrder($id){
        $r = SpsixLotteryOrder::findOne($id);                                                     
        $r->delete();
    }  
    /**
     * 获取指定期数的会员下注金额之和
    ...
    static public function getOneDayOrder($user_id, $day) {
        ...
        $query = SpsixLotteryOrder::find()
            ...
            ->innerJoin('spsix_lottery_order_sub o_sub','o.order_num=o_sub.order_num')
            ->asArray()
            ->all();
        return $query[0];
    }
    /**
     * 获取极速六合彩一天时间的最终下注金额结果
     * @param type $user_id         用户ID
     * @param type $day             查询时间
     * @return type
     */
...
//        $query = SpsixLotteryOrder::findBySql($sql)->asArray()->all();
        $query = SpsixLotteryOrder::find()
...
//    }
    ...
    static public  function getProfit($startTime,$endTime,$userId){
        //总的中奖金额
        $data_win = SpsixLotteryOrder::find()
            ->select('sum(win) as win_total')
            ->from('spsix_lottery_order_sub as o_sub')
            ->innerJoin('spsix_lottery_order as o','o.order_num=o_sub.order_num')
        ...
        //-------------------------------------------------------
        //总的反水金额
        $data_fs = SpsixLotteryOrder::find()
