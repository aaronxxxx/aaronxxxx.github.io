...
use app\modules\spsix\models\MoneyLog;
...
use app\modules\spsix\models\SysAnnouncement;
...
use app\modules\spsix\helpers\DataValid;
...
/**
    ...
    public function actionIndex() {
        ...
        $lastOne=SpsixLotterySchedule::lastOne();
        ...
        $qishu = SpsixLotterySchedule::getNewQishu(); //获取当前期数 qishu=-1 未开盘
        $kjresult = LotteryResultSplhc::getSixResult('',10);//近十期开奖结果
        if($row){
        ...
        //增加顯示生肖 20190820
        $previous = isset($lastOne['qishu']) ? $lastOne['qishu'] - 1 : 0;
        $arr = LotteryResultSplhc::getSixResultByQishu($previous);
        $Animal[] = $CommonFc->numToAnimal($arr['ball_1'], strtotime($arr['datetime']));
        $Animal[] = $CommonFc->numToAnimal($arr['ball_2'], strtotime($arr['datetime']));
        $Animal[] = $CommonFc->numToAnimal($arr['ball_3'], strtotime($arr['datetime']));
        $Animal[] = $CommonFc->numToAnimal($arr['ball_4'], strtotime($arr['datetime']));
        $Animal[] = $CommonFc->numToAnimal($arr['ball_5'], strtotime($arr['datetime']));
        $Animal[] = $CommonFc->numToAnimal($arr['ball_6'], strtotime($arr['datetime']));
        $Animal[] = $CommonFc->numToAnimal($arr['ball_7'], strtotime($arr['datetime']));

        if (!empty($getNews['showTableN'])) {$showTableN = $getNews['showTableN'];}
        if (!empty($getNews['rtype'])) {
            $rType = $getNews['rtype'];
            if($getNews['rtype'] == 'NAP'){                                     //正码过关
                $odds_NAP1 = SpsixLotteryOdds::getOdds("NAP1");
                $odds_NAP2 = SpsixLotteryOdds::getOdds("NAP2");
                $odds_NAP3 = SpsixLotteryOdds::getOdds("NAP3");
                $odds_NAP4 = SpsixLotteryOdds::getOdds("NAP4");
                $odds_NAP5 = SpsixLotteryOdds::getOdds("NAP5");
                $odds_NAP6 = SpsixLotteryOdds::getOdds("NAP6");
            ...
            if($getNews['rtype'] == 'CH'){                                      //连码
                $odds_CH = SpsixLotteryOdds::getOdds('CH');
            ...
            if($getNews['rtype'] == 'LX'){                                      //连肖
                $odds_LX2 = SpsixLotteryOdds::getOdds("LX2");
                $odds_LX3 = SpsixLotteryOdds::getOdds("LX3");
                $odds_LX4 = SpsixLotteryOdds::getOdds("LX4");
                $odds_LX5 = SpsixLotteryOdds::getOdds("LX5");
                $odds_LF2 = SpsixLotteryOdds::getOdds("LF2");
                $odds_LF3 = SpsixLotteryOdds::getOdds("LF3");
                $odds_LF4 = SpsixLotteryOdds::getOdds("LF4");
                $odds_LF5 = SpsixLotteryOdds::getOdds("LF5");
    ...
    public function actionResult() {
        $res = array();
        $tempResult = LotteryResultSplhc::getSixResult('',1);//近十期开奖结果
        $res['kjresult'] = $tempResult[0];
        return json_encode($res);
    }

    public function actionMenu(){
        $qishu = SpsixLotterySchedule::getNewQishu();  //期数 -1 是未开盘
        if($qishu==-1){$qishu="--";}
        $res= array(
            'qishu'=>"$qishu"
    ...
    public function actionSixOrder() {
        ...
        if(!isset($postNews['period'])||$postNews['period']==0){
            return $this->out(false,$postNews['period']);
        }else{
...
//            $assets, $goldArray, $oddsArray, $betInfoArray, $gid, $bet_money_one, $betInfo_one, $bet_rate_one);
    ...
    private function _getTypeSP($getNews) {
        ...
        $kjresult = LotteryResultSplhc::getSixResult('',10);//近十期开奖结果
        $row = SpsixLotterySchedule::getNewestLottery();                          //返回开盘信息
        $next_row = SpsixLotterySchedule::getNextLottery();                     //返回开盘信息
        $kaipa = $next_row['kaipan_time'];
        $now = strftime("%Y-%m-%d %H:%M:%S"); //現在時間
        $next = strtotime($kaipa) - strtotime($now); //單位秒
        if(!$row){   //还未开盘
            $result_show['isCloseAdmin']="true";
            $result_show['Msg'] =$announcement;
            $result_show['reason'] = "目前没有开盘，请咨询客服人员。";
            $result_show['kjresult'] = $kjresult;
            $result_show['next'] = $next;
            return $result_show;
        ...
        {
            $row=LotteryResultSplhc::getSixResultByQishu($qishu);
            $ball_count = 0;
            $result = '';
            $animal = '';
            if ($row){
    ...
    private function _AddSixOrder($userid, $rtype_name, $rTypeNameDetail,$rType, $lottery_number, $bet_money_total, $balance, $bet_win_total,
        ...
        $id = SpsixLotteryOrder::addSixOrder($userid, $rtype_name,$rType, $bet_info_sp, $bet_money_total, $bet_win_total, $lottery_number,$rTypeNameDetail); //添加下注订单
        if(!$id){
            return '添加订单失败';
        }
        ...
        if($r != 1){
            SpsixLotteryOrder::DelSixOrder($id);                                                      //更改失败删除订单，退出方法
            return '更新用户金额失败';
        }
        if(!$money_log_id){
            SpsixLotteryOrder::DelSixOrder($id);                                                      //更改失败删除订单，退出方法
        ...
        $r = SpsixLotteryOrder::addSixOrderOrderNumById($id,$datereg);                                    //更新插入订单的单号
        if($r != 1){
            SpsixLotteryOrder::DelSixOrder($id);                                                          //删除订单
        ...
        if($gid == 'NAP'){                                                                          //更新投注明细（下注类型等）
            ...
            if(isset($fsRow['splhc_bet']) && $fsRow['splhc_bet']){
                if ($fsRow['splhc_bet'] <= $bet_money_one) {                                              //极速六合彩反水最小金额
                    $fs_money = $bet_money_one * $fsRow['splhc_bet_reb'];
                }
            }
            ...
            $id_sub = SpsixLotteryOrderSub::AddSixOrder($datereg, $bet_info, $bet_rate, $bet_money_one, $win_money, $fs_money, $balance,$gid,$this->module->params['boll']);//添加下注明细

            if(!$id_sub){
//                SpsixLotteryOrderSub::DelSixOrder($id_sub);                                                   //删除订单明细
                SpsixLotteryOrder::DelSixOrder($id);                                                          //删除订单
                MoneyLog::DelUserMoneyLogWhenAddSixOrderErr($money_log_id);                                 //删除金额日志
                UserList::updateUserMoneyWhenMoneyLogIsErr($bet_money_total, $userid);                      //还原金额
                return '号码错误,生成订单失败';
            ...
            $r = SpsixLotteryOrderSub::UpdateSixOrder($id_sub, $datereg_sub);                              //更新插入下注明显的子订单号
            if(!$id_sub || !$r){
                SpsixLotteryOrderSub::DelSixOrder($id_sub);                                                   //删除订单明细
                SpsixLotteryOrder::DelSixOrder($id);                                                          //删除订单
                MoneyLog::DelUserMoneyLogWhenAddSixOrderErr($money_log_id);                                 //删除金额日志
            ...
            if(isset($fsRow['splhc_bet']) && $fsRow['splhc_bet']){
                if ($fsRow['splhc_bet'] <= $bet_money_one) {                                              //极速六合彩投注最小金额
                    $fs_money = $bet_money_one * $fsRow['splhc_bet_reb'];
                }
            }else{
                $fs_money = 0;
            ...
            $id_sub = SpsixLotteryOrderSub::AddSixOrder($datereg, $bet_info, $bet_rate, $bet_money_one, $win_money, $fs_money, $balance,$gid,$this->module->params['boll']);
                ...
                if($goldArray[$key]){
                    ...
                    if ($goldArray[$key]) {
                        ...
                        if (($gid == 'SP') && (0 < intval($bet_info))) {
                            $row_sp_fs = SpsixLotteryOdds::getSubTypeSPballtypeFS();
...
//                            SpsixLotteryOrderSub::DelSixOrder($id_sub);                                                   //删除订单明细
                        ...
                        $datereg_sub = date('YmdHis') . $id_sub;
                        $r = SpsixLotteryOrderSub::UpdateSixOrder($id_sub, $datereg_sub);
                        if(!$id_sub || !$r){
                            SpsixLotteryOrderSub::DelSixOrder($id_sub);                                                   //删除订单明细
                            SpsixLotteryOrder::DelSixOrder($id);                                                          //删除订单
                            MoneyLog::DelUserMoneyLogWhenAddSixOrderErr($money_log_id);                                 //删除金额日志
    ...
    private function _getState($period=0){
        $arr=SpsixLotterySchedule::lastOne();
        if($arr){
    ...
    public function actionTest($userid, $rtype_name, $rType, $lottery_number, $bet_money_total, $balance, $bet_win_total,
        ...
        $id = SpsixLotteryOrder::addSixOrder($userid, $rtype_name, $rType, $bet_info_sp, $bet_money_total, $bet_win_total, $lottery_number); //添加下注订单
        ...
        $r1 = SpsixLotteryOrder::addSixOrderOrderNumById($id,$datereg);                                    //更新插入订单的单号
        ...
        if($gid == 'NAP'){                                                                          //更新投注明细（下注类型等）
            ...
            $id_sub = SpsixLotteryOrderSub::AddSixOrder($datereg, $bet_info, $bet_rate, $bet_money_one, $win_money, $fs_money, $balance,$gid);//添加下注明细
            $datereg_sub = date('YmdHis') . $id_sub;
            $r2 = SpsixLotteryOrderSub::UpdateSixOrder($id_sub, $datereg_sub);                              //更新插入下注明显的子订单号
            ...
            $id_sub = SpsixLotteryOrderSub::AddSixOrder($datereg, $bet_info, $bet_rate, $bet_money_one, $win_money, $fs_money, $balance,$gid);
            $datereg_sub = date('YmdHis') . $id_sub;
            $r2 = SpsixLotteryOrderSub::UpdateSixOrder($id_sub, $datereg_sub);                           //更新插入下注明显的子订单号
            ...
            foreach ($goldArray as $key => $value) {
                ...
                if($goldArray[$key]){
                    ...
                    if ($goldArray[$key]) {
                        ...
                        $id_subs=array();
                        $id_subs[$key] = SpsixLotteryOrderSub::AddSixOrder($datereg, $bet_info, $bet_rate, $bet_money_one, $win_money, $fs_money, $balance,$gid);
                        $datereg_sub = date('YmdHis') . $id_subs[$key];
                        $rs[$key] = SpsixLotteryOrderSub::UpdateSixOrder($id_subs[$key], $datereg_sub);
...
//=================================================================================下注的部分类型的验证处理
    ...
    private  function  OrderValidSP($gid,$goldArray,$oddsArray){
        if ($gid == 'SP') {
            $odds_SP = SpsixLotteryOdds::getOddsByBallType('SP', 'a_side');
        } else if ($gid == 'SPbside') {
            $odds_SP = SpsixLotteryOdds::getOdds('SP');
        }
        $odds_SP_other = SpsixLotteryOdds::getOddsByBallType('SP', 'other');
        for ($i = 1; $i < 50; $i++) {
    ...
    private function OrderValidNAS($gid,$goldArray,$oddsArray){
        $oddsN = SpsixLotteryOdds::getOdds($gid);
        for ($i = 1; $i < 50; $i++) {
            $numString = ($i < 10 ? '0' . $i : $i);
    ...
    private function OrderValidNA($gid,$goldArray,$oddsArray){
        $odds_NA = SpsixLotteryOdds::getOdds('NA');
        $odds_NA_other = SpsixLotteryOdds::getOddsByBallType('NA', 'other');
        for ($i = 1; $i < 50; $i++) {
            $numString = ($i < 10 ? '0' . $i : $i);
            if (0 < $goldArray[$gid . $numString]) {
    ...
    private function OrderValidNO($gid,$goldArray,$oddsArray){
        $odds1_other = SpsixLotteryOdds::getOddsByBallType('N1', 'other');
        $odds2_other = SpsixLotteryOdds::getOddsByBallType('N2', 'other');
        $odds3_other = SpsixLotteryOdds::getOddsByBallType('N3', 'other');
        $odds4_other = SpsixLotteryOdds::getOddsByBallType('N4', 'other');
        $odds5_other = SpsixLotteryOdds::getOddsByBallType('N5', 'other');
        $odds6_other = SpsixLotteryOdds::getOddsByBallType('N6', 'other');
        for ($i = 1; $i < 7; $i++) {
            if ($i == 1) {
    ...
    private function OrderValidSPA($gid,$goldArray,$oddsArray){
        $odds_SP_other = SpsixLotteryOdds::getOddsByBallType('SP', 'other');
        $odds_SPA = SpsixLotteryOdds::getOdds('SPA');
        for ($i = 1; $i < 10; $i++) {
    ...
    private function OrderValidC7($gid,$goldArray,$oddsArray){
        $odds_C7 = SpsixLotteryOdds::getOdds('C7');
        for ($i = 1; $i < 10; $i++) {
    ...
    private function OrderValidSPB($gid,$goldArray,$oddsArray){
        $odds_SPB = SpsixLotteryOdds::getOdds('SPB');
        for ($i = 1; $i < 10; $i++) {
    ...
    private  function OrderValidHB($gid,$goldArray,$oddsArray){
        $odds_HB = SpsixLotteryOdds::getOdds('HB');
    ...
    private function OrderNAP($postNews,$goldArray){
        for ($i=1;$i<=6;$i++){
            $odds_NAP[$i] = SpsixLotteryOdds::getOdds('NAP'.$i);
        }
        for ($i =1;$i<=6;$i++){
...
//                $bet_win_total = $bet_win_total + ($bet_money_one * $oddsValue); //20190729 因應php7以上修改
    ...
    private function OrderLX($postNews,$bet_money_total,$bet_win_total){
        $gid = trim($postNews['gid']);
        for ($i=2;$i<=5;$i++){
            $odds_LX[$i] = SpsixLotteryOdds::getOdds('LX'.$i);
            $odds_LF[$i] = SpsixLotteryOdds::getOdds('LF'.$i);
        }
    ...
    private function OrderOther($goldArray,$bet_money_total,$bet_win_total,$userid,$oddsArray,$gid,$qishu){
        ...
        $money_already = SpsixLotteryOrder::getMaxMoneyAlready_lhc($userid, $qishu);
        if(!$money_already){
            $max_money_already=0;
        }else{
    ...
    private function ShowSPbside($rtype,$rTypeN,$showTableN,$fengpanTime,$row,$differTime,$announcement,$kjresult,$kaijiangTime){
        ...
        $res = array('BetLineD' => 'N', 'sTime' => $fengpanTime, 'other_close' => '0', 'gID' => 'SPbside', 'Line_M' => '4',
            ...
            'SP_OVER' => $odds_SP_other['h3'],
            'SP_UNDER' => $odds_SP_other['h4'],
            'SP_SODD' => $odds_SP_other['h10'],
            'SP_SEVEN' => $odds_SP_other['h6'],
            'SP_SOVER' => $odds_SP_other['h7'],
            'SP_SUNDER' => $odds_SP_other['h8'],
            'SF_OVER' => $odds_SP_other['h9'],
            'SF_UNDER' => $odds_SP_other['h5'],
            'SP_R' => $odds_SP_other['h11'],
            'SP_G' => $odds_SP_other['h12'],
            'SP_B' => $odds_SP_other['h13'],
            'HS_OO' => $odds_SP_other['h14'],
    ...
    private function ShowOEOU($rtype,$rTypeN,$showTableN,$fengpanTime,$row,$differTime,$announcement,$kjresult,$kaijiangTime){
        for ($i=1;$i<=6;$i++){
            $odds_other[$i] = SpsixLotteryOdds::getOddsByBallType('N'.$i, 'other');
        }
        $odds_NA_other = SpsixLotteryOdds::getOddsByBallType('NA', 'other');
        $odds_SP_other = SpsixLotteryOdds::getOddsByBallType('SP', 'other');
        $res = array(
    ...
    private function ShowNAS($rtype,$rTypeN,$showTableN,$fengpanTime,$row,$differTime,$announcement,$kjresult,$kaijiangTime){
        $oddsN = SpsixLotteryOdds::getOdds($rTypeN);
        $res = array(
            "BetLineD" => "N",
            "sTime" => $fengpanTime,
            "gID" => " $rTypeN ",
