...
function updateOpenTime() {
    $.post("class/odds_gdsf.php", function (data) {
        if (data.isopen > 0 && ((data.opentime > 70 && data.opentime < 500) || (data.opentime < 50 && data.opentime > 20))) {
            clearTimeout(fp);
...
function loadinfo() {
    $.post("class/odds_gdsf.php", function (data) {
        if (data.opentime > 0) {
            ...
            $.jBox.alert('当前彩票已经封盘，请稍后再进行下注！<br><br>广东快乐十分开盘时间为：09:00 - 23:00', '提示');
            return false;
...
//更新开奖号码
function auto(ball) {
    $.post("class/auto_gdsf.php", {ball: ball}, function (data) {
