...
*/
    ...
    // 快速下注
        ...
        $(".fast-lhai ul li").click(function() {
            ...
            $('#table1 .fast-sel').next().next().next().children('input').val($fastMoneySum);
        })
        // 选球事件
        $("#table1").on("click","td", function() {
            if($(".fast-lhai ul li").hasClass('fast-sel') == false) {
                $(this).siblings().eq($(this).index()+2).children("input").val();
            }
            // 判断选球
            if($(this).index() == 0) {
                ...
                if ($(this).hasClass('fast-sel') == true) {
                    // 将选中金额赋值给input框
                    $(this).siblings().eq($(this).index()+2).children("input").val($fastMoneySum);
                    // 判断只有自定义输入金额时
                    if($(".fast-lhai ul li").hasClass('fast-sel') == false) {
                        $(this).siblings().eq($(this).index()+2).children("input").val($mineMoney);
                    }                    
                } else {
                    $(this).siblings().eq($(this).index()+2).children("input").val("");
                }
        ...
        $('#fast-input-lhai').bind('input propertychange', function () {
            $mineMoney= parseInt($("#fast-input-lhai").val());
            $('#table1 .fast-sel').next().next().next().children('input').val($mineMoney);
            $('#table2 .fast-sel').next().next().next().children('input').val($mineMoney);
            if ($(this).val() == "") {
                $('#table1 .fast-sel').next().next().next().children('input').val("");
                $('#table2 .fast-sel').next().next().next().children('input').val("");
            };
        });
        // 自定义金额输入框失焦事件
        ...
        $("#res1").click(function() {
            ...
            $('#table1 .fast-sel').next().next().children('input').val("");
            $fastMoney = 0;
            $mineMoney="";
            $fastMoneySum = "";
        })
