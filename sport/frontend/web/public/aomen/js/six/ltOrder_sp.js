...
//        	alert("准备下注");
 ...
 //           	alert("绑定下注提交事件");
        ...
        onOkClk : function (me) {
            ...
            if (G == '') {
                me._goldInp.focus();
                layer.alert(Lang('pls_key_amount'));
                return false
            }
            if (isNaN(parseInt(G)) == true) {
                me._goldInp.focus();
                layer.alert(Lang('MSG_ONLY_INPUT_NUMBER'));
                return false
            }
            G = parseInt(G, 10);
            if (G < me.gMin) {
                me._goldInp.focus();
                layer.alert(Lang('amount_under'));
                return false
            }
            if (G > me.gMax) {
                me._goldInp.focus();
                layer.alert(Lang('amount_max') + ' : ' + me.gMax);
                return false
            }
            if (G > me.SO) {
                me._goldInp.focus();
                layer.alert(Lang('amount_sg_bet'));
                return false
            }
            if (G > me.credit) {
                me._goldInp.focus();
                layer.alert(Lang('amount_ov_credit'));
                return false
            }
            var _sel = selectBall();
            if (_sel.length == 0) {
                layer.alert(Lang('Msg2_Quick'));
                return false;
            }
        ...
        onBetAClk : function (me) {
            ...
            for ( var k in me._goldGrp) {
                ...
                if (_gold != '') {
                    if (!_reg.test(_gold)) {
                        layer.alert(Lang('MSG_IntegerOnly'));
                        return false;
                    }
                    _win += _gold * (_odds - 1);
                    ...
                    if(parseInt(_gMin) > _gold){
                        layer.alert(Lang('amount_under') + ' : '  + _gMin);
                        return false;
                    }
                    if(parseInt(_gMax) < _gold){
                        layer.alert(Lang('amount_max') + ' : '  + _gMax);
                        return false;
                    }
                }
            }
            ...
            if (_win == 0) {
                layer.alert(Lang('MSG_PLZ_CHK_UnChked'));
                return false;
            }
            _win = _win.toFixed(2);
        ...
        onBetChk : function () {
            ...
            if (G < this.gMin) {
                this._goldInp.focus();
                layer.alert(Lang('amount_under') + ' : ' + this.gMin);
                return false;
            }
            if (G > this.gMax) {
                this._goldInp.focus();
                layer.alert(Lang('amount_max') + ' : ' + this.gMax);
                return false;
            }

            if (this._winGoldTag.innerHTML > 1000000 && this.millionLimit) {
                layer.alert(Lang('max_payout'));
                return false;
            }
            if (G > this.credit) {
                this._goldInp.focus();
                ...
                $.ajax({
                    ...
                    success: function(response){
                        if(response.msg=='未登录，请先登录'){
                            ...
                            layer.alert(Lang('OperateSuccess'));
                            $("#message_box").css('display', 'none');
                            document.getElementById("bet-credit").innerHTML = response.msg;
			...
			layer.confirm(
				Lang('word_bet_gold') + ":" + G*selectNumber,
				{btn:['确定','取消'],closeBtn:0},
				_fn,
				function(index){
					layer.close(index);
...
//        	alert("正码过关下注开始");
            ...
            if (this.teamcount <= 1) {
                layer.alert(Lang('ov_oneteam'));
                return false;
            }
            if (this.teamcount >= 9) {
                layer.alert(Lang('overflow_oneteam'));
                return false;
            }
            ...
            _ok = this._btnSend, _no = this._btnCancel, _form = this._form, me = this, _fn = function () {
				layer.closeAll();
                _ok.disabled = true;
                _no.disabled = true;
                me.lock = true;
                $.ajax({
                    ...
                    error : function () {
                        _ok.disabled = false;
                        _no.disabled = false;
                        //layer.alert(Lang('NetworkAnomaly'));
						layer.confirm('还未登录,是否现在登录？',{btn:['确定','取消']},
							function(){
								var $url=window.location.href;
								var $index=$url.indexOf('/?r=');
        ...
        onGrpSend : function (me) {
            ...
            me._goldGrp.each(function () {
                ...
                if (G < me.gMin) {
                    ...
                    me._btnSend.attr('disabled', false);
                    this.focus();
                    layer.alert(Lang('amount_under') + ' : ' + me.gMin);
                    _err++;
                    return false
                }
                if (G > me.gMax) {
...
//                    me._btnSend.attr('disabled', false);
//                    this.focus();
//                    layer.alert(Lang('amount_sg_bet') + ' : ' + me.SO);
//                    _err++;
//                    return false
//                }
                if (G > me.credit) {
                    ...
                    me._btnSend.attr('disabled', false);
                    this.focus();
                    layer.alert(Lang('amount_ov_credit') + ' : ' + me.credit);
                    _err++;
                    return false
                }
                _G += parseInt(G, 10);
