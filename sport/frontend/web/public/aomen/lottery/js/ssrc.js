...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.ssrc.opentime > 0 && !(nowTime < '09:58' && nowTime > '04:00')) {
            $("#open_qihao").html(data.ssrc.number);
            endtime(data.ssrc.opentime);
            auto(1);
            $('#resultOpenTime').parent('.resultTime').html('距下期开奖仅有:' + '&nbsp;<span id="resultOpenTime"></span>')
            if (un != 0) {
                clearInterval(un);
            }
            ...
            $(".tabLabel").html('' +
                ...
                '<div class="open-date" style="font-size: 36px; font-weight: bold;">' + y + "/" + m + "/" + d + '　' + '10:00' + '</div>' +
                '</div>')
        } else {
            is_fenpan = true;
            $(".bian_td_odds").html("-");
...
function auto(ball) {
    var html = '',
        res_str = '';
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if ($("#numbers").html() != data.ssrc.numbers) {
            $('#numbers').html(data.ssrc.numbers);
            for (var i = 0; i < data.ssrc.hm.length; i++) {
                res_str += '<li class="ballc_' + data.ssrc.hm[i] + '">' + data.ssrc.hm[i] + '</li>';
            }
            $('#autoinfo').html(res_str);
        }
        if ($("#open_qihao").html() - data.ssrc.numbers != 1) {
...
function order() {
    ...
    $.post("/?r=lottery/lzssrc/index/prepare-order", {
        ...
        if (data.status == false) {
            ...
            console.log(data.summoney + '/' + data.sumbetball);
            layer.open({
                skin: 'layer_confirm',
                title: [''],
                area: ['90%', 'auto'],
