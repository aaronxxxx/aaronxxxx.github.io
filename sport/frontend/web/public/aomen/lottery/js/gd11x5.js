...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.gd11.opentime > 0 && nowTime > '08:50' && nowTime < '23:10') {
            $("#open_qihao").html(data.gd11.number);
            endtime(data.gd11.opentime);
            auto(1);
            if (un != 0) {
                clearInterval(un);
            }
            ...
            $(".tabLabel").html('' +
                ...
                '<div class="open-date" style="font-size: 36px; font-weight: bold;">' + y + "/" + m + "/" + d + '　' + '09:10' + '</div>' +
                '</div>')
        } else {
            is_fenpan = true;
            $(".bian_td_odds").html("-");
...
function auto(ball) {
    var res_str = '';
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if ($("#numbers").html() != data.gd11.numbers) {
            $("#numbers").html(data.gd11.numbers);
            for (var i = 0; i < data.gd11.hm.length; i++) {
                res_str += '<li class="ballc_' + data.gd11.hm[i] + '">' + data.gd11.hm[i] + '</li>';
            }
            $('#autoinfo').html(res_str);
        }
        if ($("#open_qihao").html() - data.gd11.numbers != 1) {
...
function order() {
    ...
    $.post("/?r=lottery/lzgd11/index/prepare-order", {
        ...
        if (data.status == false) {
            ...
            //         layer.close(index);
            //         $.post("/?r=lottery/lzgd11/index/insert-order", {data: betjsonstr, _csrf: csrfToken}, function (data) {
            //             if (data.code == 10) {
