...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.pk10.opentime > 0 && nowTime > '08:50' && nowTime < '23:50') {
            $("#open_qihao").html(data.pk10.number);
...
function auto(ball) {
    var html = '',
        res_str = '';
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if ($("#numbers").html() != data.pk10.numbers) {
            $('#numbers').html(data.pk10.numbers);
            for (var i = 0; i < data.pk10.hm.length; i++) {
                res_str += '<li class="ballc_' + data.pk10.hm[i] + '">' + data.pk10.hm[i] + '</li>';
            }
            $('#autoinfo').html(res_str);
        }
        if ($("#open_qihao").html() - data.pk10.numbers != 1) {
