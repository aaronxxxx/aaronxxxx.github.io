...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.mlaft.opentime > 0 && !(nowTime < '13:04' && nowTime > '04:04')) {
            $("#open_qihao").html(data.mlaft.number);
            ...
            $(".tabLabel").html('' +
                ...
                '<div class="open-date" style="font-size: 36px; font-weight: bold;">' + y + "/" + m + "/" + d + '　' + '13:04' + '</div>' +
                '</div>')
        } else {
            is_fenpan = true;
            $(".bian_td_odds").html("-");
...
function auto(ball) {
    var html = '',
        res_str = '';
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if ($("#numbers").html() != data.mlaft.numbers) {
            $('#numbers').html(data.mlaft.numbers);
            for (var i = 0; i < data.mlaft.hm.length; i++) {
                res_str += '<li class="ballc_' + data.mlaft.hm[i] + '">' + data.mlaft.hm[i] + '</li>';
            }
            $('#autoinfo').html(res_str);
        }
        if ($("#open_qihao").html() - data.mlaft.numbers != 1) {
