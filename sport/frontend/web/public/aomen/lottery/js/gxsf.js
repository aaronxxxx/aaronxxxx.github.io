...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.gxsf.opentime > 0 && nowTime > '08:40' && nowTime < '22:20') {
            $("#open_qihao").html(data.gxsf.number);
            endtime(data.gxsf.opentime);
            auto(1);
            if (un != 0) {
                clearInterval(un);
            }
...
function auto(ball) {
    var res_str = '';
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if ($("#numbers").html() != data.gxsf.numbers) {
            $("#numbers").html(data.gxsf.numbers);
            for (var i = 0; i < data.gxsf.hm.length; i++) {
                res_str += '<li class="ballc_' + data.gxsf.hm[i] + '">' + data.gxsf.hm[i] + '</li>';
            }
            $('#autoinfo').html(res_str);
        }
        if ($("#open_qihao").html() - data.gxsf.numbers != 1) {
...
function order() {
    ...
    $.post("/?r=lottery/lzgxsf/index/prepare-order", {
        ...
        if (data.status == false) {
            ...
            //     layer.close(index);
            //     $.post("/?r=lottery/lzgxsf/index/insert-order", {data: betjsonstr, _csrf: csrfToken}, function (data) {
            //         if (data.code == 10) {
