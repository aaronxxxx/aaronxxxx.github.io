...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.shssl.opentime > 0 && nowTime > '10:00' && nowTime < '21:30') {
            $("#open_qihao").html(data.shssl.number);
            endtime(data.shssl.opentime);
            auto(1);
            if (un != 0) {
                clearInterval(un);
            }
            ...
            $(".tabLabel").html('' +
                ...
                '<div class="open-date" style="font-size: 36px; font-weight: bold;">' + y + "/" + m + "/" + d + '　' + '10:30' + '</div>' +
                '</div>')
        } else {
            is_fenpan = true;
            $(".bian_td_odds").html("-");
...
function auto(ball) {
    var res_str = '';
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if ($("#numbers").html() != data.shssl.numbers) {
            $("#numbers").html(data.shssl.numbers);
            for (var i = 0; i < data.shssl.hm.length; i++) {
                res_str += '<li class="ball_' + data.shssl.hm[i] + '">' + data.shssl.hm[i] + '</li>';
            }
            $('#autoinfo').html(res_str);
        }
        if ($("#open_qihao").html() - data.shssl.numbers != 1) {
            xhm = setTimeout("auto(1)", 30000);
        }
    }, "json");
}
...
function order() {
    ...
    $.post("/?r=lottery/lzshssl/index/prepare-order", {
        ...
        if (data.status == false) {
            ...
            //         layer.close(index);
            //         $.post("/?r=lottery/lzshssl/index/insert-order", {data: betjsonstr, _csrf: csrfToken}, function (data) {
            //             if (data.code == 10) {
