...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.tjsf.opentime > 0 && nowTime > '08:30' && nowTime < '22:50') {
            $("#open_qihao").html(data.tjsf.number);
            endtime(data.tjsf.opentime);
            auto(1);
            if (un != 0) {
                clearInterval(un);
            }
            ...
            $(".tabLabel").html('' +
                ...
                '<div class="open-date" style="font-size: 36px; font-weight: bold;">' + y + "/" + m + "/" + d + '　' + '08:50' + '</div>' +
                '</div>')
        } else {
            is_fenpan = true;
            $(".bian_td_odds").html("-");
...
function auto(ball) {
    var res_str = '';
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if ($("#numbers").html() != data.tjsf.numbers) {
            $("#numbers").html(data.tjsf.numbers);
            for (var i = 0; i < data.tjsf.hm.length; i++) {
                res_str += '<li class="ballc_' + data.tjsf.hm[i] + '">' + data.tjsf.hm[i] + '</li>';
            }
            $('#autoinfo').html(res_str);
        }
        if ($("#open_qihao").html() - data.tjsf.numbers != 1) {
...
function order() {
    ...
    $.post("/?r=lottery/lztjsf/index/prepare-order", {
        ...
        if (data.status == false) {
            ...
            //         layer.close(index);
            //         $.post("/?r=lottery/lztjsf/index/insert-order", {data: betjsonstr, _csrf: csrfToken}, function (data) {
            //             if (data.code == 10) {
