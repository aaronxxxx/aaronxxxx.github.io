...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.orpk.opentime > 0 && nowTime > '10:00' && nowTime < '23:59') {
            $("#open_qihao").html(data.orpk.number);
        ...
        } else if (data.orpk.fengpan <= 0 && !(nowTime > '10:00' && nowTime < '23:59')) {
            $('#tabinnerBet').html('' +
...
function auto(ball) {
    var html = '',
        res_str = '';
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if ($("#numbers").html() != data.orpk.numbers) {
            $('#numbers').html(data.orpk.numbers);
            for (var i = 0; i < data.orpk.hm.length; i++) {
                res_str += '<li class="ballc_' + data.orpk.hm[i] + '">' + data.orpk.hm[i] + '</li>';
            }
            $('#autoinfo').html(res_str);
        }
        if ($("#open_qihao").html() - data.orpk.numbers != 1) {
