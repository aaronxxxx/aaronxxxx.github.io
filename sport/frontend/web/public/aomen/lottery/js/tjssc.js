...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.tjssc.opentime > 0) {
            $("#open_qihao").html(data.tjssc.number);
...
function auto(ball) {
    var res_str = "";
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if ($("#numbers").html() != data.tjssc.numbers) {
            $("#numbers").html(data.tjssc.numbers);
            for (var i = 0; i < data.tjssc.hm.length; i++) {
                res_str += '<li class="ballc_' + data.tjssc.hm[i] + '">' + data.tjssc.hm[i] + '</li>';
            }
            $('#autoinfo').html(res_str);
        }
        if ($("#open_qihao").html() - data.tjssc.numbers != 1) {
