...
function loadinfo() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.pl3.opentime > 0) {
            $("#open_qihao").html(data.pl3.number);
            endtime(data.pl3.opentime);
            auto(1);
            if (un != 0) {
                clearInterval(un);
            }
...
function auto(ball) {
    var res_str = '';
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        $("#numbers").html(data.pl3.numbers);
        for (var i = 0; i < data.pl3.hm.length; i++) {
            res_str += '<li class="ballc_' + data.pl3.hm[i] + '">' + data.pl3.hm[i] + '</li>';
        }
...
function order() {
    ...
    $.post("/?r=lottery/lzpl3/index/prepare-order", {
        data: betjsonstr,
        _csrf: csrfToken
    }, function (data) {
        ...
        if (data.status == false) {
            ...
            layer.open({
                ...
                yes: function (index) {
                    ...
                    removeBet();
                    layer.close(index);
                    $.post("/?r=lottery/lzpl3/index/insert-order", {
                        data: betjsonstr,
                        _csrf: csrfToken
                    }, function (data) {
                        if (data.code == 10) {
            ...
            //         layer.close(index);
            //         $.post("/?r=lottery/lzpl3/index/insert-order", {data: betjsonstr, _csrf: csrfToken}, function (data) {
            //             if (data.code == 10) {
