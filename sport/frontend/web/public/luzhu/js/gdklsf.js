...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.gdsfc.opentime, data.gdsfc.fengpan);
        if (data.gdsfc.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.gdsfc.numbers) {
            twoside();
            dropmap();
            var html = '';
            for (var i = 0; i < data.gdsfc.hm.length; i++) {
                html += '<span id="result' + data.gdsfc.hm[i] + '" class="number num' + data.gdsfc.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.gdsfc.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    ...
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.gdsfc.opentime > 0 && nowTime > '08:40' && nowTime < '23:00') {
            ...
            $("#currGameNo").html(data.gdsfc.number);
            twoside();
            dropmap();
...
//两面长龙
function twoside() {
    $('#changlong').load('/?r=lottery/lzgdsf/index/ajaxchanglong');
}
...
function dropmap() {
    if ($type == 1 && $("#count_isbig").hasClass('kon') || $type == 0 && $("#count_issingle").hasClass('kon')) {
        $('#lz_disp').load('/?r=lottery/lzgdsf/index/order-list', {
            'w': $w,
            'tp': $type
        });
    }
...
function longhumap() {
    $('#lz_disp').load('/?r=lottery/lzgdsf/index/longhu', {
        'longhu': $longhu
    });
}
...
//单球露珠图
function luzhutu() {
    $('#lz_disp').load('/?r=lottery/lzgdsf/index/luzhutu', {
        'w': $w,
        'tp': $type
    });
}
