...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.gxsf.opentime, data.gxsf.fengpan);
        if (data.gxsf.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.gxsf.numbers) {
            twoside();
            dropmap();
            var html = '';
            for (var i = 0; i < data.gxsf.hm.length; i++) {
                html += '<span id="result' + data.gxsf.hm[i] + '" class="number num' + data.gxsf.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.gxsf.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    fenpan();
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.gxsf.opentime > 0 && nowTime > '08:40' && nowTime < '22:20') {
            un = setInterval(fenpan, 3000);
            $("#currGameNo").html(data.gxsf.number);
            twoside();
            dropmap();
        } else if (!(nowTime > '08:40' && nowTime < '22:20')) {
            $('#html_content').append('<div class="mask"><div class="maskBox"><div class="maskImg"></div><span class="maskMsg">目前休盘，请期待下一盘</span><hr><span class="maskOpen">下此开奖时间：<span class="maskTime">次日09:00</span></span><div></div>');
        } else {
            $(".bet_odds").html("-");
            $(".amount").html("封盘");
...
//两面长龙
function twoside() {
    $('#changlong').load('/?r=lottery/lzgxsf/index/ajaxchanglong');
}
...
function dropmap() {
    if ($type == 1 && $("#gxsf_count_isbig").hasClass('kon') || $type == 0 && $("#gxsf_count_issingle").hasClass('kon')) {
        $('#lz_disp').load('/?r=lottery/lzgxsf/index/order-list', {
            'w': $w,
            'tp': $type
        });
    }
...
function longhumap() {
    $('#lz_disp').load('/?r=lottery/lzgxsf/index/longhu', {
        'longhu': $longhu
    });
}
...
//单球露珠图
function luzhutu() {
    $('#lz_disp').load('/?r=lottery/lzgxsf/index/luzhutu', {
        'w': $w,
        'tp': $type
    });
}
...
function order() {
    ...
    $.post("/?r=lottery/lzgxsf/index/prepare-order", {
        data: betjsonstr
    }, function (data) {
        $('.btn-red').attr("disabled", false);
        if (data.status == false) {
            ...
            layer.confirm("共 ￥" + data.summoney + " / " + data.sumbetball + " 笔，确定下注吗？<br />下注明细如下：<br />" + data.ballmsg,
                function () {
                    layer.closeAll();
                    cq_cancel();
                    $.post("/?r=lottery/lzgxsf/index/insert-order", {
                        data: betjsonstr
                    }, function (data) {
                        if (data.code == '10') {
                            layer.msg("操作成功", {
