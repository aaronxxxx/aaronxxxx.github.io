...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.fc3d.opentime, data.fc3d.fengpan);
        if (data.fc3d.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.fc3d.numbers) {
            var html = '';
            for (var i = 0; i < data.fc3d.hm.length; i++) {
                html += '<span id="result' + data.fc3d.hm[i] + '" class="number num' + data.fc3d.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.fc3d.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    ...
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.fc3d.opentime > 0) {
            ...
            $("#currGameNo").html(data.fc3d.number);
        } else {
            $(".bet_odds").html("-");
            $(".amount").html("封盘");
...
function order() {
    ...
    $.post("/?r=lottery/lzfc3d/index/prepare-order", {data: betjsonstr}, function (data) {
        $('.btn-red').attr("disabled",false);
        if(data.status==false){
            ...
            layer.confirm("共 ￥" + data.summoney + " / " + data.sumbetball + " 笔，确定下注吗？<br />下注明细如下：<br />" + data.ballmsg,
                function(){
                    layer.closeAll();
                    cq_cancel();
                    $.post("/?r=lottery/lzfc3d/index/insert-order", {data: betjsonstr}, function (data) {
                        if (data.code == '10') {
