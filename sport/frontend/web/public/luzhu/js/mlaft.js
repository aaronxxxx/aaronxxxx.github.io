...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.mlaft.opentime, data.mlaft.fengpan);
        if (data.mlaft.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.mlaft.numbers) {
            twoside();
            dropmap();
            var html = '';
            for (var i = 0; i < data.mlaft.hm.length; i++) {
                html += '<span id="result' + data.mlaft.hm[i] + '" class="number num' + data.mlaft.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.mlaft.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    fenpan();
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.mlaft.opentime > 0 && !(nowTime < '13:04' && nowTime > '04:04')) {
            un = setInterval(fenpan, 3000);
            $("#currGameNo").html(data.mlaft.number);
            twoside();
            dropmap();
        } else if (nowTime < '13:04' && nowTime > '04:04') {
            $('#html_content').append('<div class="mask"><div class="maskBox"><div class="maskImg"></div><span class="maskMsg">目前休盘，请期待下一盘</span><hr><span class="maskOpen">下此开奖时间：<span class="maskTime">次日13:04</span></span><div></div>');
        } else {
            $(".bet_odds").html("-");
            $(".amount").html("封盘");
...
//两面长龙
function twoside() {
    $('#changlong').load('/?r=lottery/lzmlaft/index/ajaxchanglong');
}
...
function dropmap() {
    $('#lz_disp').load('/?r=lottery/lzmlaft/index/order-list', {
        'w': $w,
        'tp': $type
    });
}
...
function order() {
    ...
    $.post("/?r=lottery/lzmlaft/index/prepare-order", {
        data: betjsonstr
    }, function (data) {
        $('.btn-red').attr("disabled", false);
        if (data.status == false) {
            ...
            layer.confirm("共 ￥" + data.summoney + " / " + data.sumbetball + " 笔，确定下注吗？<br />下注明细如下：<br />" + data.ballmsg,
                function (index) {
                    cq_cancel();
                    layer.close(index);
                    $.post("/?r=lottery/lzmlaft/index/insert-order", {
                        data: betjsonstr
                    }, function (data) {
                        if (data.code == '10') {
                            layer.msg("操作成功", {
