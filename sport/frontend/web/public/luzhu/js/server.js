...
function videoType() {
    $(window).on('load', function () {
        var videoCheck = $('#select_sys li.c').attr('data-video');
        if (videoCheck == '1') {
            $('#videoBtn').show();
        } else {
            $('#videoBtn').hide();
...
//判别彩种
function clickVideo() {
    var videoType = $('#select_sys li.c').find('h3').html();
    switch (videoType) {
        case '重庆时时彩':
            event.preventDefault();
            window.parent.ssccqVideo();
            break;
        case '北京PK拾':
            event.preventDefault();
            window.parent.bjpk10Video();
            break;
        case '老PK拾':
            event.preventDefault();
            window.parent.orpkVideo();
            break;
        case '极速时时彩':
            event.preventDefault();
            window.parent.tjsscVideo();
            break;
        case '极速赛车':
            event.preventDefault();
            window.parent.fkscVideo();
            break;
        case '幸运飞艇':
            event.preventDefault();
            window.parent.mlaftVideo();
            break;
    }
}