...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.ts5.opentime, data.ts5.fengpan);
        if (data.ts5.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.ts5.numbers) {
            twoside();
            dropmap();
            var html = '';
            for (var i = 0; i < data.ts5.hm.length; i++) {
                html += '<span id="result' + data.ts5.hm[i] + '" class="number num' + data.ts5.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.ts5.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    ...
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.ts5.opentime > 0) {
            ...
            $("#currGameNo").html(data.ts5.number);
            twoside();
            dropmap();
        } else {
            $(".bet_odds").html("-");
...
//两面长龙
function twoside() {
    $('#changlong').load('/?r=lottery/lzts5/index/ajaxchanglong');
}
...
function dropmap() {
    if ($type == 1 && $("#count_isbig").hasClass('kon') || $type == 0 && $("#count_issingle").hasClass('kon')) {
        $('#lz_disp').load('/?r=lottery/lzts5/index/order-list', {
            'w': $w,
            'tp': $type
        });
    }
...
//单球露珠图
function luzhutu() {
    $('#lz_disp').load('/?r=lottery/lzts5/index/luzhutu', {
        'w': $w,
        'tp': $type
    });
}
...
function longhumap() {
    $('#lz_disp').load('/?r=lottery/lzts5/index/longhu', {
        'longhu': $longhu
    });
}
