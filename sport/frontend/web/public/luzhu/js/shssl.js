...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.shssl.opentime, data.shssl.fengpan);
        if (data.shssl.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.shssl.numbers) {
            var html = '';
            for (var i = 0; i < data.shssl.hm.length; i++) {
                html += '<span id="result' + data.shssl.hm[i] + '" class="number num' + data.shssl.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.shssl.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    fenpan();
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.shssl.opentime > 0 && nowTime > '10:00' && nowTime < '21:30') {
            un = setInterval(fenpan, 3000);
            $("#currGameNo").html(data.shssl.number);
        } else if (!(nowTime > '10:00' && nowTime < '21:30')) {
            $('#html_content').append('<div class="mask"><div class="maskBox"><div class="maskImg"></div><span class="maskMsg">目前休盘，请期待下一盘</span><hr><span class="maskOpen">下此开奖时间：<span class="maskTime">次日10:30</span></span><div></div>');
        } else {
            $(".bet_odds").html("-");
            $(".amount").html("封盘");
...
function order() {
    ...
    $.post("/?r=lottery/lzshssl/index/prepare-order", {
        data: betjsonstr
    }, function (data) {
        $('.btn-red').attr("disabled", false);
        if (data.status == false) {
            ...
            layer.confirm("共 ￥" + data.summoney + " / " + data.sumbetball + " 笔，确定下注吗？<br />下注明细如下：<br />" + data.ballmsg,
                function () {
                    layer.closeAll();
                    cq_cancel();
                    $.post("/?r=lottery/lzshssl/index/insert-order", {
                        data: betjsonstr
                    }, function (data) {
                        if (data.code == '10') {
                            layer.msg("操作成功", {
