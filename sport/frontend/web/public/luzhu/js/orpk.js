...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.orpk.opentime, data.orpk.fengpan);
        if (data.orpk.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.orpk.numbers) {
            twoside();
            dropmap();
            var html = '';
            for (var i = 0; i < data.orpk.hm.length; i++) {
                html += '<span id="result' + data.orpk.hm[i] + '" class="number num' + data.orpk.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.orpk.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    fenpan();
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.orpk.opentime > 0 && nowTime > '09:55' && nowTime < '23:59') {
            un = setInterval(fenpan, 3000);
            $("#currGameNo").html(data.orpk.number);
            twoside();
            dropmap();
        } else if (data.orpk.fengpan <= 0 && !(nowTime > '09:55' && nowTime < '23:59')) {
            $('#html_content').append('<div class="mask"><div class="maskBox"><div class="maskImg"></div><span class="maskMsg">目前休盘，请期待下一盘</span><hr><span class="maskOpen">下此开奖时间：<span class="maskTime">次日10:00</span></span><div></div>');
        } else {
            $(".bet_odds").html("-");
            $(".amount").html("封盘");
...
//两面长龙
function twoside() {
    $('#changlong').load('/?r=lottery/lzorpk/index/ajaxchanglong');
}
//露珠图异步刷新
$w = 1, $type = 1;

function dropmap() {
    $('#lz_disp').load('/?r=lottery/lzorpk/index/order-list', {
        'w': $w,
        'tp': $type
    });
}
...
function order() {
    ...
    $.post("/?r=lottery/lzorpk/index/prepare-order", {
        data: betjsonstr
    }, function (data) {
        $('.btn-red').attr("disabled", false);
        if (data.status == false) {
            ...
            layer.confirm("共 ￥" + data.summoney + " / " + data.sumbetball + " 笔，确定下注吗？<br />下注明细如下：<br />" + data.ballmsg,
                function (index) {
                    cq_cancel();
                    layer.close(index);
                    $.post("/?r=lottery/lzorpk/index/insert-order", {
                        data: betjsonstr
                    }, function (data) {
                        if (data.code == '10') {
                            layer.msg("操作成功", {
