...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.gd11.opentime, data.gd11.fengpan);
        if (data.gd11.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.gd11.numbers) {
            twoside();
            dropmap();
            var html = '';
            for (var i = 0; i < data.gd11.hm.length; i++) {
                html += '<span id="result' + data.gd11.hm[i] + '" class="number num' + data.gd11.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.gd11.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    fenpan();
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.gd11.opentime > 0 && nowTime > '08:50' && nowTime < '23:10') {
            un = setInterval(fenpan, 3000);
            $("#currGameNo").html(data.gd11.number);
            twoside();
            dropmap();
        } else if (!(nowTime > '08:50' && nowTime < '23:10')) {
...
//两面长龙
function twoside() {
    $('#changlong').load('/?r=lottery/lzgd11/index/ajaxchanglong');
}
...
function dropmap() {
    if ($type == 1 && $("#count_isbig").hasClass('kon') || $type == 0 && $("#count_issingle").hasClass('kon')) {
        $('#lz_disp').load('/?r=lottery/lzgd11/index/order-list', {
            'w': $w,
            'tp': $type
        });
    }
...
function longhumap() {
    $('#lz_disp').load('/?r=lottery/lzgd11/index/longhu', {
        'longhu': $longhu
    });
}
...
//单球露珠图
function luzhutu() {
    $('#lz_disp').load('/?r=lottery/lzgd11/index/luzhutu', {
        'w': $w,
        'tp': $type
    });
}
...
function order() {
    ...
    $.post("/?r=lottery/lzgd11/index/prepare-order", {
        data: betjsonstr
    }, function (data) {
        $('.btn-red').attr("disabled", false);
        if (data.status == false) {
            ...
            layer.confirm("共 ￥" + data.summoney + " / " + data.sumbetball + " 笔，确定下注吗？<br />下注明细如下：<br />" + data.ballmsg,
                function () {
                    layer.closeAll();
                    cq_cancel();
                    $.post("/?r=lottery/lzgd11/index/insert-order", {
                        data: betjsonstr
                    }, function (data) {
                        if (data.code == '10') {
                            layer.msg("操作成功", {
