...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.cqsfc.opentime, data.cqsfc.fengpan);
        if (data.cqsfc.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.cqsfc.numbers) {
            twoside();
            dropmap();
            var html = '';
            for (var i = 0; i < data.cqsfc.hm.length; i++) {
                html += '<span id="result' + data.cqsfc.hm[i] + '" class="number num' + data.cqsfc.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.cqsfc.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    ...
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.cqsfc.opentime > 0) {
            ...
            $("#currGameNo").html(data.cqsfc.number);
            twoside();
            dropmap();
        } else {
            $(".bet_odds").html("-");
...
//两面长龙
function twoside() {
    $('#changlong').load('/?r=lottery/lzcqsf/index/ajaxchanglong');
}
...
function dropmap() {
    if ($type == 1 && $("#count_isbig").hasClass('kon') || $type == 0 && $("#count_issingle").hasClass('kon')) {
        $('#lz_disp').load('/?r=lottery/lzcqsf/index/order-list', {
            'w': $w,
            'tp': $type
        });
    }
...
function longhumap() {
    $('#lz_disp').load('/?r=lottery/lzcqsf/index/longhu', {
        'longhu': $longhu
    });
}
...
//单球露珠图
function luzhutu() {
    $('#lz_disp').load('/?r=lottery/lzcqsf/index/luzhutu', {
        'w': $w,
        'tp': $type
    });
}
