...
function fenpan() {
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        ...
        clearTimeout(fp);
        endtime(data.cqssc.opentime, data.cqssc.fengpan);
        if (data.cqssc.opentime > 30) {

        } else {
            clearInterval(un);
        }
        if ($("#prevGameNo").text() != data.cqssc.numbers) {
            twoside();
            dropmap();
            var html = '';
            for (var i = 0; i < data.cqssc.hm.length; i++) {
                html += '<span id="result' + data.cqssc.hm[i] + '" class="number num' + data.cqssc.hm[i] + '"></span>'
            }
            $(".numresult").html(html);
            $("#prevGameNo").html(data.cqssc.numbers);
        }
    }, "json");
}
...
function loadinfo() {
    fenpan();
    $.get("file/lottery.json?_=" + Math.random(), function (data) {
        if (data.cqssc.opentime > 0 && !(nowTime < '06:50' && nowTime > '03:10')) {
            un = setInterval(fenpan, 3000);
            $("#currGameNo").html(data.cqssc.number);
            twoside();
            dropmap();
        } else if (nowTime < '06:50' && nowTime > '03:10') {
            $('#html_content').append('<div class="mask"><div class="maskBox"><div class="maskImg"></div><span class="maskMsg">目前休盘，请期待下一盘</span><hr><span class="maskOpen">下此开奖时间：<span class="maskTime">次日07:10</span></span><div></div>');
        } else {
            $(".bet_odds").html("-");
            $(".amount").html("封盘");
...
//两面长龙
function twoside() {
    $('#changlong').load('/?r=lottery/lzcqssc/index/ajaxchanglong');
}
...
function dropmap() {
    if ($type == 1 && $("#count_isbig").hasClass('kon') || $type == 0 && $("#count_issingle").hasClass('kon')) {
        $('#lz_disp').load('/?r=lottery/lzcqssc/index/order-list', {
            'w': $w,
            'tp': $type
        });
    }
...
//单球露珠图
function luzhutu() {
    $('#lz_disp').load('/?r=lottery/lzcqssc/index/luzhutu', {
        'w': $w,
        'tp': $type
    });
}
...
//总和龙虎
function longhumap() {
    $('#lz_disp').load('/?r=lottery/lzcqssc/index/longhu', {
        'longhu': $longhu
    });
}
...
function order() {
    ...
    $.post("/?r=lottery/lzcqssc/index/prepare-order", {
        data: betjsonstr
    }, function (data) {
        $('.btn-red').attr("disabled", false);
        if (data.status == false) {
            ...
            layer.confirm("共 ￥" + data.summoney + " / " + data.sumbetball + " 笔，确定下注吗？<br />下注明细如下：<br />" + data.ballmsg,
                function () {
                    layer.closeAll();
                    cq_cancel();
                    $.post("/?r=lottery/lzcqssc/index/insert-order", {
                        data: betjsonstr
                    }, function (data) {
                        if (data.code == '10') {
                            layer.msg("操作成功", {
