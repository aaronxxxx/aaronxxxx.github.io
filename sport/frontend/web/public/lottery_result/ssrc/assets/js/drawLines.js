...
ESUNChart.ini = {
	...
	default_spaceLine: true,
	default_yl_Line: true,
	map: [],
	initShow: "",
	stop_buy_re: /sina|tenpay|youa/,
	no_show_news: /paipai|youa/
};
...
var scrwidth = $("body").width(),
	scrheigjt = window.screen.height,
	moveheight = "";
//      alert(scrwidth)
JoinLine.prototype = {
	show: function (b) {
...
Cookie = {
	...
	each: function (f) {
		...
		for (var c = 0; c < d.length; ++c) {
			var b = d[c].split("="),
				g = f.call(this, this.trim(b[0]), decodeURIComponent(b[1]), c);
			if (g !== undefined) {
				return g;
			}
		}
...
userLine = {
	...
	IE: function (x1, y1, x2, y2) {
		var D = document,
			Line = D.body.appendChild(D.createElement("<esun:line style=\"z-index:3;position:absolute;left:0;top:0\"><esun:Stroke dashstyle=\"shortdot\" endarrow=\"classic\" /></esun:line>"));
		with(this.Line = Line) {
			from = x1 + "," + y1;
			to = x2 + "," + y2;
