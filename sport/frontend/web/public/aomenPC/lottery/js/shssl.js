...
function updateOpenTime() {
	$.get("/?r=lottery/shssl/index/get-front-info",function(data){
        if (data.isopen > 0 && ((data.opentime > 126 && data.opentime < 1500) || (data.opentime < 110 && data.opentime > 20))) {
            clearTimeout(fp);
...
function loadinfo() {
	$.get("/?r=lottery/shssl/index/get-front-info",function(data){
        if (data.opentime > 0) {
            ...
            $.jBox.alert('当前彩票已经封盘，请稍后再进行下注！<br><br>上海时时乐开盘时间为：每日10:00 - 22:00', '提示');
            return false;
        }
    }, "json");
}
...
//更新开奖号码
function auto(ball) {
	$.get("/?r=lottery/shssl/index/get-k-j-result", function(data){
...
//    }
    ...
    $.post("/?r=lottery/shssl/index/prepare-order",{data:betjsonstr,_csrf:csrfToken},function(data){
    	...
    	if(data.code=='1'){
        	...
        	layer.confirm(data.msg, {btn:['确认','取消']}, 
            		function(index){
		    			document.orders.reset();
		    			layer.close(index);
		    			$.post("/?r=lottery/shssl/index/insert-order",{data:betjsonstr,_csrf:csrfToken}, function(data){
		    				if(data==null){
