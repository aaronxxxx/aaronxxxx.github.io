...
function updateOpenTime() {
	$.get("/?r=lottery/gd11/index/get-front-info",function(data){
        if (data.isopen > 0 && ((data.opentime > 66 && data.opentime < 500) || (data.opentime > 20 && data.opentime < 50))) {
            clearTimeout(fp);
...
function loadinfo() {
	$.get("/?r=lottery/gd11/index/get-front-info",function(data){
        if (data.opentime > 0) {
            ...
            $.jBox.alert('当前彩票已经封盘，请稍后再进行下注！<br><br>广东11选5开盘时间为：每日09:00 - 次日02:00', '提示');
            return false;
        }
    }, "json");
}
...
//更新开奖号码
function auto(ball) {
	$.get("/?r=lottery/gd11/index/get-k-j-result", function(data){
...
function order() {
    ...
    $.post("/?r=lottery/gd11/index/prepare-order",{data:betjsonstr,_csrf:csrfToken},function(data){
    	...
    	if(data.code=='1'){
        	...
        	layer.confirm(data.msg, {btn:['确认','取消']}, 
            		function(index){
		    			document.orders.reset();
		    			layer.close(index);
		    			$.post("/?r=lottery/gd11/index/insert-order",{data:betjsonstr,_csrf:csrfToken}, function(data){
		    				if(data==null){
