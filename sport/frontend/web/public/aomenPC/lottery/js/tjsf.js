...
function updateOpenTime() {
	$.get("/?r=lottery/tjsf/index/get-front-info",function(data){
...
function loadinfo() {
	$.get("/?r=lottery/tjsf/index/get-front-info",function(data){
        if (data.opentime > 0) {
            ...
            $.jBox.alert('当前彩票已经封盘，请稍后再进行下注！<br><br>天津快乐十分开盘时间为：09:00 - 23:00', '提示');
            return false;
...
//更新开奖号码
function auto(ball) {
	$.get("/?r=lottery/tjsf/index/get-k-j-result", function(data){
...
function order() {
    ...
    $.post("/?r=lottery/tjsf/index/prepare-order",{data:betjsonstr,_csrf:csrfToken},function(data){
    	...
    	if(data.code=='1'){
        	...
        	layer.confirm(data.msg, {btn:['确认','取消']}, 
            		function(index){
		    			document.orders.reset();
		    			layer.close(index);
		    			$.post("/?r=lottery/tjsf/index/insert-order",{data:betjsonstr,_csrf:csrfToken}, function(data){
		    				if(data==null){
