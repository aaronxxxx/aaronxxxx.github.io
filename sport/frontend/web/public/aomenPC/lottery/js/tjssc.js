...
function updateOpenTime() {
	$.get("/?r=lottery/tjssc/index/get-front-info",function(data){
        if (data.isopen > 0 && ((data.opentime > 126 && data.opentime < 500) || (data.opentime < 110 && data.opentime > 20))) {
            clearTimeout(fp);
...
function loadinfo() {
	$.get("/?r=lottery/tjssc/index/get-front-info",function(data){
        if (data.opentime > 0) {
            ...
            $.jBox.alert('当前彩票已经封盘，请稍后再进行下注！<br><br>极速时时彩开盘时间为：全天24小时', '提示');
            return false;
        }
    }, "json");
}
...
//更新开奖号码
function auto(ball) {
	$.get("/?r=lottery/tjssc/index/get-k-j-result", function(data){
...
//    }
    ...
    $.post("/?r=lottery/tjssc/index/prepare-order",{data:betjsonstr,_csrf:csrfToken},function(data){
    	...
    	if(data.code=='1'){
        	...
        	layer.confirm(data.msg, {btn:['确认','取消']}, 
            		function(index){
		    			document.orders.reset();
		    			layer.close(index);
		    			$.post("/?r=lottery/tjssc/index/insert-order",{data:betjsonstr,_csrf:csrfToken}, function(data){
		    				if(data==null){
