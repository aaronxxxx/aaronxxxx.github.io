    ...
    GameSpace.ChShow.prototype = {
        ...
        onFormSubmit : function (me) {
            ...
            if (_count < me.minChk && !isSub) {
                ...
                // $("#message_box").load(this.action, function () {
                ...
                //     $("#res").on("click",function(){$("#message_box").css("display","none");});
                // });
                $.ajax({
                    url:this.action,
                    dataType : 'json',
        ...
        ajaxXF : function(X, F) {
		    ...
		    if((X=='undefined')||(F=='undefined')){
                ...
                var a1Array = '';
                if(_resultX[X]){
                    $.each(_resultX[X],function (key,val) {
                        a1Array = val.split(",");
                    })
                }
                for(var i2=0;i2<a1Array.length;i2++){
                    for(var i3=0;i3<_resultF[F].length;i3++){
                        if(parseInt(a1Array[i2]) == parseInt(_resultF[F][i3])){
                            a2.splice(i3,1);
                        }
                    }
                }
                ...
                for(var i2=0;i2<a1Array.length;i2++){
                    for(var i3=0;i3<a2.length;i3++){
                        if(parseInt(a1Array[i2])!=parseInt(a2[i3])){
                            totalArray.push(a1Array[i2] + ", " + a2[i3]);
                        }
                    }
                }
                var pageString = '<p>组合共 <font id="TotalBall" color="red">'+totalArray.length+'</font> 组</p>';
        ...
        dantuoChk : function (k) {
            ...
            if ((_total > 286 && min > 2) || (_total > 253 && min == 2)) {
                ...
                $('input[name=DantuoCancel]').click();//组合超出数量后，清除选中球号
