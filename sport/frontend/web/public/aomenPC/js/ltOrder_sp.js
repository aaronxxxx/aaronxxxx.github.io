...
(function(space){
    ...
    var _opt = {
        url : { 
            back : '/?r=spsix/nap/tuichu', 
            bet : '/?r=spsix/nap/bet-view',
            nap : '/?r=spsix/index/six-order'
        },
        reg : {
            gold : /^gold\[([^\]]*)\]$/,
            odds : /^odds\[([^\]]*)\]$/,
...
//    }
    ...
    space.bet.prototype = {
        ...
        onBetAClk : function (me) {
            ...
            if($('input[name=is_login]').val()=='' || $('input[name=is_login]').val()=='undefined' || $('input[name=is_login]').val()==null){
                new_alert('请先登录');
                return false;
            }
            ...
            var _btn = this, _this = me, _fn = function () {
                ...
                me.lock = true;
                $.ajax({
                    url : _this._form.action + "&t="+new Date().getTime(),
                    type : 'POST',
                    ...
                    success: function(response){
                        ...
                        $.ajax({
                            ...
                            dataType: "json",
                            success: function(response){
                                $("#user_money",parent.parent.document).html(response.data.user.money);
                                setTimeout(function(){location.reload();},1500);
                            }
                        })
                        new_alert(response.msg);
                        // document.getElementById("bet-credit").innerHTML = response;
                        if(typeof(Left) != "undefined" ){
                            setTimeout(function(){window.location.reload();},1500);//页面刷新
                        }
                    }
                });
            };
                    ...
                    success: function(response){
                        ...
                        $.ajax({
                            ...
                            success: function(response){
                                ...
                                setTimeout(function(){location.reload();},1500);//页面刷新
                            }
                        })
                        new_alert(response.msg);
                        document.getElementById("bet-credit").innerHTML = response;
...
$(document).mouseup(function (e) {
    var container =$("#chipChangeBox"); // 這邊放你想要排除的區塊
