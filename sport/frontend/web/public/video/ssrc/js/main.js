...
function countdowntimer(timer) {
	...
	if(timer == 2) {
		if(ifopen){
			$("#sound").attr("src", "sound/cuttime.mp3");
		}
		document.getElementById("sound").pause();
		if(ifopen && videoTools.ifsund()) {
			document.getElementById("sound").play();
...
function startanimation() { //開始跑
	setTimeout(function() {
		$("#sound").attr("src", "sound/staring.mp3");
		document.getElementById("sound").pause();
		if(ifopen && videoTools.ifsund()) {
...
function finishgame(finalresult) { //結束遊戲?

	//console.log("startFinish"+finalresult);
	setTimeout(function() {
		if(ifopen){
...
function winnerpage(results) {
	...
	for(var i = 0; i < 3; i++) {
		...
		$(position).attr("src", "images/winner" + resultarry[i] + ".png");
	}
