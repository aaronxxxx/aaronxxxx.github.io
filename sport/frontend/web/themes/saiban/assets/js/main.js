...
function handicap() {
	...
	$('.container').each(function () {
		if ($(this).attr('data-type') == 'pk-mode') {
			$('#sport_content .pk-mode .lists .lists-group .media').each(function () {
				var $comity = $(this).find('.comity p span'),
					comity = $comity.text(),
					comity_val = parseFloat(comity.split('W', 1));
				if (Math.sign(comity_val) < 0) { // 負數 -1 , 0, 正數 1
					$(this).find('.comity img').addClass('arrow-reverse')
					$comity.text(-comity_val + 'W')
				}
			});
		} else if ($(this).attr('data-type') == 'multi-mode') {
			$('#sport_content .multi-mode .lists .lists-group .media').each(function () {
				var $comity = $(this).find('.rangfen'),
					comity = $comity.text(),
					comity_val = parseFloat(comity.split('W', 1));
				if (Math.sign(comity_val) < 0) { // 負數 -1 , 0, 正數 1
					$comity.text(-comity_val + 'W')
...
function layerButton() {
	$('body').on('click', '[data-button=layer]', function () {
		var bodyType = $(this).parents('.container').attr('data-type'),
			...
			Multi_Banner = $(this).parents('.media').find('.media-header').css('background');
		...
		layer.open({
			...
			success: function () {
				...
				$('#betting_list [name=betting_dollars]').keyup(function () {
					...
					$('#betting_list .betting_money').html($('#betting_list [name=betting_dollars]').val() * $('#betting_list [data-id=odds]').text());
				});
				// 點擊送出功能
...
var sport = {
	init: function init() {
		$.get("/themes/saiban/assets/yuntsai/files/new.json", function (data) {
			...
			setTimeout(() => {
				handicap();
				var $comity = $('#sport_content .pk-mode .lists .lists-group .media .comity');
				$comity.find('img').css('opacity', 1)
				$comity.find('p span').css('opacity', 1)
			}, 10);
...
$(window).on('load', function () {
	...
	// click 觸發 layer_tips
	$('[data-button="layer_tip"]').on('click', function () {
		...
		switch (id) {
			case 'display_record':
				...
				layer.tips(message, '[data-button-id=' + id + ']', option);
				break;
		}
	});
});